...
== Create a Composite Vector Index Using SQL++
////
...
For a full list of these parameters, see xref:n1ql:n1ql-language-reference/createindex.adoc#index-with[WITH Clause] in the {sqlpp} for Query Reference.
...
For a full list of the parameters that affect a Composite Vector index, see xref:n1ql:n1ql-language-reference/createindex.adoc[].
...
The following query gets a single document from the `rgb` collection in the `vector-sample` bucket's `color` scope.
...
The following query gets a single document from the `rgb_questions` collection in the `vector-sample` bucket's `color` scope.
...
You can click the btn:[View] button to see and copy the entire query with all the vectors.
