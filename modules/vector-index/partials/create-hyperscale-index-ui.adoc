...
. In the *Type* list, select *Dense* if the vector field contains dense embeddings, or *Sparse* if the vector field contains sparse term-weight pairs.
...
. If the vector field has not yet been populated with data, select *Index Without Data*.
...
.. (Dense vector indexes only) In the *Similarity Metric* list, select the similarity metric that you want to use for comparing vectors in your index.
...
.. (Dense vector indexes only) In the *Quantization* list, select the type of quantization to use for simplifying the vectors stored in your index.
+
For more information about how to choose quantization on your index, see <<quantization-setting>>.

.. [[dimensions]](Dense vector indexes only) In the *Dimensions* field, enter the exact dimension of the vectors in your data.
+
...
.. (Optional, dense vector indexes only) In the *Number of centroids* field, enter or select the number of centroids to create in your index.
+
...
.. (Dense vector indexes only) To store full vectors in the index, select *Persist Full Vector*.
+
