...
[source,sqlpp]
----
include::example$prepared/elixir-prep-statements.n1ql[tag=twonamed]
----
...
[source,sqlpp]
----
include::example$prepared/elixir-prep-statements.n1ql[tag=twonumbered]
----
...
[source,sqlpp]
----
include::example$prepared/elixir-prep-statements.n1ql[tag=twounnumbered]
----
...
* If you are using the Query Workbench, use the btn:[options] button to display the Query Options window, set the parameters, then run the statement.
...
// These includes are currently broken in the devex repo as the files do not exist.
// Refer to https://docs.couchbase.com/server/current/guides/prep-statements.html for what should be here.
//'''
