...
[source,sqlpp]
----
include::server:settings:example$param-numbers.n1ql[tags=**]
----
...
[source,sqlpp]
----
include::server:settings:example$param-names.n1ql[tags=**]
----

<1> In the cbq shell, parameter names must be prefixed by a hyphen.
<2> In the cbq shell, the REST API, or the Query Workbench, named parameters must begin with a dollar sign `$`.

For more information and examples, refer to xref:server:settings:query-settings.adoc[Settings and Parameters].
--
