...
The command line clients also provide a quick and streamlined interface for access and are suitable if you just want to access an item without writing any code.
For this guide, we're especially interested in the `cbimport` tool.
...
If you're using the *Reactor API* then, as OpenCSV does not have a built-in converter,
...
If you're using the *Reactor API* then, once you have read the JSON array, use the https://projectreactor.io/docs/core/release/api/reactor/core/publisher/Flux.html#fromIterable-java.lang.Iterable-[`Flux::fromIterable`] method to convert it into streams:
...
If you're using the *Reactor API* then open the JSONL file as a stream using the https://projectreactor.io/docs/core/release/api/reactor/core/publisher/Flux.html#using-java.util.concurrent.Callable-java.util.function.Function-java.util.function.Consumer-[`Flux::using`] method.
...
Now decide which bucket and xref:tutorials:buckets-scopes-and-collections.adoc[scope and collection] you want to import to, and create them if they do not already exist.
...
For more information, see xref:dotnet-sdk:howtos:managing-connections.adoc[].
--
...
If you're using the xref:java-sdk:howtos:concurrent-async-apis.adoc#reactive-programming-with-reactor[Reactive API], then use the reactive collection instead:
...
For more information, see xref:java-sdk:howtos:managing-connections.adoc[].
--
...
For more information, see xref:nodejs-sdk:howtos:managing-connections.adoc[].
--
...
For more information, see xref:python-sdk:howtos:managing-connections.adoc[].
--
====
...
. Specify the key.
This could be as easy as extracting the `id` field from the document, or using an incrementing sequence number.
...
This means that in the case of any error, it's easy to make any required tweaks to the import file and re-run the whole import.
...
NOTE: As CsvHelper and Newtonsoft generate different outputs, we have provided some overloaded options that work for either.
...
For more information, see xref:dotnet-sdk:howtos:kv-operations.adoc[].
--
...
For more information, see xref:java-sdk:howtos:kv-operations.adoc[].
--
...
For more information, see xref:nodejs-sdk:howtos:kv-operations.adoc[].
--
...
For more information, see xref:python-sdk:howtos:kv-operations.adoc[].

The Python SDK offers a set of batch operations which are marked as volatile as of SDK 3.2.3, which may be more efficient.
Here's a brief example for CSV:
...
| xref:cxx-sdk:howtos:kv-operations.adoc[{cpp}]
...
| xref:kotlin-sdk:howtos:kv-operations.adoc[Kotlin]
...
| xref:rust-sdk:howtos:kv-operations.adoc[Rust]
