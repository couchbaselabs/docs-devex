...
You can create replicas of primary indexes, secondary indexes, and secondary index partitions, to enhance index availability.
// end::intro[]
...
If you do not specify the number of partitions, the index has 8 partitions.
...
For more information and examples, see xref:n1ql:n1ql-language-reference/index-partitioning.adoc[Index Partitioning].
...
NOTE: If you specify both `num_replica` and `nodes`, the number of nodes must be one greater than the number of replicas.

For more information and examples, see xref:indexes:index-replication.adoc#index-replication[Index Replication].

== Altering Index Placement

You can alter the placement of an existing index or replica, increase or decrease the number of replicas, or to drop a replica temporarily.
...
For more information and examples, see xref:n1ql:n1ql-language-reference/alterindex.adoc[ALTER INDEX].
