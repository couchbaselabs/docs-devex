...
For more information, see {nushell-api-url}/reject.html[reject for filters^] in the Nushell documentation.
--
...
NOTE: If the path does not exist, the SDK will return a `PathNotFoundException` error.
...
NOTE: If the path does not exist, the SDK will return a `PathNotFoundError` error.
