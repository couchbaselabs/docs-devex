...
+
--
To get index recommendations for a single query, do the following:

* Enter the query in the Query Editor.
* Open the *Indexes* tab.

The Indexes tab displays the details of any current indexes used by the query, and any indexes that the Index Advisor recommends.

Click btn:[Build Suggested] to create any recommended indexes.

image::elixir-index-advisor.png["Index advice for the query"]

For more information about the index advisor, see xref:cloud:clusters:query-service/query-workbench.adoc#index-advisor[Index Advisor].
--
