...
To connect to a standalone or Docker installation with _unencrypted_ network access, set up a user with xref:server:learn:security/roles.adoc[appropriate access levels] and a secure password.
...
To connect to a cluster configured with TLS (Transport Layer Security), for _encrypted_ network access, you must supply an xref:server:learn:security/certificates.adoc[X.509 certificate].
