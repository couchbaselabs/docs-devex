...
To access a cluster via the Couchbase Server Web Console over an unencrypted connection, navigate to the Web Console address with your browser (by default, `pass:[http://localhost:8091]`) and enter your credentials.
...
To access a cluster via the Couchbase Server Web Console over an encrypted connection, navigate to the secure Web Console address with your browser (by default, `pass:[https://localhost:18091]`) and enter your credentials.
...
Couchbase Server can be configured to run with unencrypted or encrypted network access. 
When running Couchbase in a production environment, the latter is always recommended.

[#basic-auth]
=== Basic Authentication

To connect to a standalone or Docker installation with unencrypted network access, set up a user with xref:learn:security/roles.adoc[appropriate access levels] and a secure password.
...
. Provide the bucket name required in the connection URL (couchbase://localhost/<bucket-name>).
...
For more information, see {dotnet-api-url}/Couchbase.Cluster.html[Cluster^].
--
...
[source,java]
----
include::java-sdk:devguide:example$java/ManagingConnections.java[tag=simpleconnect,indent=0]
----
...
For more information, see {java-api-url}/Cluster.html[Cluster^].
--
...
For more information, see {nodejs-api-url}/Cluster.html[Cluster^].
--
...
For more information, see {python-api-url}#cluster-object[Cluster^].
--
====
...
To connect to a cluster configured with TLS (Transport Layer Security), for encrypted network access, you must supply an xref:learn:security/certificates.adoc[X.509 certificate].
...
. Provide the bucket name required in the connection URL (couchbases://127.0.0.1/<bucket-name>).
The scheme "couchbases://" implies an encrypted connection is used.
...
[source,csharp]
----
include::dotnet-sdk:howtos:example$Auth.csx[tag=auth,indent=0]
----
...
[source,java]
----
include::java-sdk:devguide:example$java/Auth.java[tag=certauth,indent=0]
----
...
For more information, see {python-api-url}#cluster-object[Cluser^].
--
====
...
| xref:cxx-sdk:howtos:managing-connections.adoc[{cpp}]
...
| xref:kotlin-sdk:howtos:connecting.adoc[Kotlin]
...
| xref:rust-sdk:howtos:managing-connections.adoc[Rust]
