...
Retrieving documents by ID is the fastest and simplest way to read xref:server:learn:data/data.adoc[data] in Couchbase.
The xref:server:learn:services-and-indexes/services/data-service.adoc[Key-Value (KV) or Data Service] allows you to retrieve a full document when you need to fetch all of the data stored. 
...
[source,console]
----
cbc-subdoc -u Administrator -P password -U couchbase://localhost/$DATABASE_NAME
