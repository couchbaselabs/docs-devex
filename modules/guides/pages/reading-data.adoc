...
However, in instances where this can be costly and unnecessary, Couchbase also provides access to specific paths within a document.
...
+
--
. If you have not already done so, use `cb-env` to set the bucket, scope, and collection where the document is stored.
...
NOTE: If the document does not exist, the SDK returns a `DocumentNotFoundException` error.
...
For more information, see {java-api-url}/Collection.html[Collection^].
--
...
NOTE: If the document does not exist, the SDK returns a `DocumentNotFoundError` error.
...
Instead, a better practice would be to access specific paths, or Sub-Documents, to perform more efficient read operations.
...
For more information, see {nushell-api-url}/get.html[get for filters^] in the Nushell documentation.
--
...
NOTE: If the document path cannot be found, the SDK returns a `PathNotFoundException` error.
...
| xref:cxx-sdk:howtos:subdocument-operations.adoc[{cpp}]
...
| xref:rust-sdk:howtos:subdocument-operations.adoc[Rust]
