...
The query context enables you to specify a scope to resolve partial keyspace references within your queries.
...
. In the *context* drop down, expand the bucket where you want to run your query.
. Select a scope.

image::elixir-query-context-set.png["The query context menu with `$DATABASE_NAME.samples` selected"]
--
...
//For example, the following command sets the query context to `$DATABASE_NAME.samples`.
...
+
--
To unset the query context:

. Go to *Data Tools* > *Query*.
. In the *context* drop down, expand the currently selected bucket.
. Click the currently selected scope.

The context drop down menu will disappear.

image::elixir-query-context-unset.png["The query context menu with the query context unset"]
--
...
====
The following query gets the name and city of every airport. Note that this assumes the query context is set to `samples`.
