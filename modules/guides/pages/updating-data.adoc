...
If it does not exist, Couchbase Capella creates a new document.
...
NOTE: If the document does not exist, the SDK will return a `DocumentNotFoundException` error.
...
NOTE: If the document does not exist, the SDK will return a `DocumentNotFoundError` error.
...
You can use either a Sub-Document upsert or replace operation depending on what's required for your application.
...
For more information, see {nushell-api-url}/upsert.html[upsert for filters^] or {nushell-api-url}/update.html[update for filters^] in the Nushell documentation.
--
