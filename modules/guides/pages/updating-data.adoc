...
. Pipe the output, which includes the document ID and the updated content, through the `doc replace` command to update the document.
...
[source,sh]
----
cinclude::example$kv/kv-subdoc.nu[tag=cbsh-subdoc-upsert]
----
