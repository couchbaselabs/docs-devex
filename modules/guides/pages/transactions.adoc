...
To create a transaction using {sqlpp} in Couchbase Capella, you must use the xref:n1ql:n1ql-intro/cbq.adoc[cbq shell].
...
You can access transaction settings and parameters through
ifdef::flag-devex-query[]
any of the usual Query tools, such as the Query tab or
endif::flag-devex-query[]
the cbq shell.
...
For the new named parameter, enter a name in the *name* box and a value in the *value* box.
