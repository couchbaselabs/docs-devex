...
. To specify any other parameters, click btn:[+] in the corresponding *Named Parameters* or *Positional Parameters* section.
...
image::elixir-transaction-settings.png['The Query Options window, with Scan Consistency set to "not_bounded", Transaction Timeout set to "120", and named parameter "durability_level" set to "none"']
...
+
--
To execute a statement as a transaction, turn on the *transaction* toggle, enter the statement in the Query Editor and click btn:[Run Query].
...
For further information on executing transactions, see xref:n1ql:n1ql-language-reference/transactions.adoc#query-tools[Query Tools].
...
. When you have entered the entire transaction, click btn:[Run Query] to execute the transaction.
...
Second, create a new keyspace for the transactions to run in:
...
Second, set the query context to the `tenants` scope in the sample data.
