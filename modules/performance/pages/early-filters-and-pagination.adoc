...
Index xref:performance:index_pushdowns.adoc[pushdowns] provide one way to reduce the amount of data to be fetched.
...
In Couchbase Server, [def]_early filtering_, [def]_early ordering_, and [def]_early pagination_ provide another line of defense, in cases where covering indexes and index pushdowns are not available.
