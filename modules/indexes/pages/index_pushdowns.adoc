...
:page-aliases: learn:services-and-indexes/indexes/index_pushdowns,performance:index_pushdowns
...
The Query engine tries to use the indexer functionality as much as possible by pushing down operations to the indexer as part of the index scan.
...
Each index has document field names explicitly specified as index-keys, and some metadata fields, such as `meta().id`, implicitly stored in the index.
The Query engine requests the exact list of fields needed for the query.
For a covered query, it's the fields referred in projection, predicate, GROUP BY clauses, ORDER BY clauses, HAVING clauses, ON key, and subqueries of the query.
For non-covered queries, it's just `META().id`.
...
[source,json]
----
include::example$pushdown-simple.jsonc[tag=projection]
----

The query refers to fields `sourceairport` and `destinationairport`.
The index is wider in scope: it has `sourceairport`, `destinationairport`, and `schedule.day` fields.

For each matching document, the query requires only a subset of the data stored in the index.
...
In some cases (such as when `distinctScan` or `intersectScan` are used, as in this example), the primary key `meta().id` may be retrieved even though the query does not explicitly specify it in the query.
...
For example, in the above query <<example-simple>> with a WHERE clause, the predicate (`sourceairport = "SFO"`) is pushed to the index `def_inventory_route_route_src_dst_day` with the following single `span` and `range`.
...
[source,json]
----
include::example$pushdown-simple.jsonc[tag=predicate]
----
...
* For more information, see xref:index-scans.adoc[].
====
...
When the predicate is conjunctive with multiple `AND` conditions, then a single span with multiple ranges is specified in the index-scan request.
When the predicate is disjunctive, then multiple spans are specified.
See xref:index-scans.adoc[] for more details and examples on how predicate pushdown works for various types of index-scans, as well as the conjunctive predicate `AND` and the disjunctive predicate `OR`.
...
image::IndexKeyOrder.png["Order of keys in a composite index",570]
...
[source,json]
----
include::example$pushdown-comp-explain.jsonc[tag=excerpt]
----
...
[source,json]
----
include::example$pushdown-comp-plan.jsonc[tags=index;ellipsis;final]
----
====
...
In Couchbase Server 6.5 and later, it's possible to push down a composite predicate on non-continuous leading index-keys.
...
<1> This query has a range predicate on the first index-key `distance` and an equality predicate on the third index-key `destinationairport`.
The query has no predicate on the 2nd index-key `sourceairport`.

.Results
[source,json]
----
include::example$pushdown-skip.jsonc[tag=excerpt]
----

<.> The EXPLAIN query plan output shows that the predicate is pushed-down to the indexer.
<.> The first element of `range[]` corresponds to the index-key `distance` with (`low`, `high`) values (`null`, `2000`) respectively.
<.> The second element of `range[]` corresponds to the index-key `sourceairport` with no low or high bounds -- in other words, a whole range scan.
...
* Whether or not the SELECT query has other clauses that may affect pagination, such as ORDER BY or JOIN.
For example,
...
[source,json]
----
include::example$pushdown-page-1.jsonc[tag=excerpt]
----
...
[source,json]
----
include::example$pushdown-page-2.jsonc[tag=excerpt]
----
...
[source,json]
----
include::example$pushdown-page-3.jsonc[tags=index;ellipsis;order]
----
...
[source,json]
----
include::example$pushdown-idx-1.jsonc[tags=index;ellipsis;query]
// ...
----

<1> In this example, the query plan does not have an ORDER operator before the final projection.
...
[source,json]
----
include::example$pushdown-idx-2.jsonc[tags=index;ellipsis;query]
----

<1> In this example, an additional ORDER operator appears before the final projection, because the ORDER BY field `meta().id` is different from the index order key `city`.
====
...
Reverse-scan of the index is not supported.
...
However, the `ASC` order in <<example-idx-1>> will not be able to use the index order in the index `idx_landmark_city_desc`, nor will the DESC order in <<example-idx-3>> be able to use the index order in the index `def_inventory_landmark_city`.
...
[source,json]
----
include::example$pushdown-idx-3.jsonc[tags=index;ellipsis;query]
----
====
...
This function returns the highest value of the input field based on the default collation rules.
(For details, see xref:n1ql:n1ql-language-reference/datatypes.adoc[] and xref:n1ql:n1ql-language-reference/comparisonops.adoc[].)
...
[source,json]
----
include::example$pushdown-max.jsonc[tag=excerpt]
----
====

=== MIN() Pushdown

This function returns the lowest value of the input field based on the default collation rules.
(For details, see xref:n1ql:n1ql-language-reference/datatypes.adoc[] and xref:n1ql:n1ql-language-reference/comparisonops.adoc[].)
...
[source,json]
----
include::example$pushdown-min.jsonc[tag=excerpt]
----
====
...
[source,json]
----
include::example$pushdown-count.jsonc[tag=excerpt]
----
...
[source,json]
----
include::example$pushdown-distinct.jsonc[tag=excerpt]
----
