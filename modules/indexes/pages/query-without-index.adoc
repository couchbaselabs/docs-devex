...
When you run a sequential scan, the Index Service may create temporary files called spill files.
In Couchbase Server {totoro} and later, spill files are encrypted when encryption at rest is enabled.
...
The query takes one or more seconds.
====
