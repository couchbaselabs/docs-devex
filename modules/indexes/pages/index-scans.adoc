...
:page-aliases: learn:services-and-indexes/indexes/index-scans,performance:index-scans
:imagesdir: ../images
...
Query predicates indicate the subsets of data that you are interested in.
During the query planning phase, you select the indexes to be used.
Then, for each index, you decide the predicates to be applied by each index.
...
In this LIKE predicate '%' is the leading portion of the string, so you can't push any portion of the string to the index service.
