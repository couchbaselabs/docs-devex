...
[.labels]
[.status]#Couchbase Server {totoro}#

The Index Service supports encryption at rest in Couchbase Server {totoro} and later.
For more information, see xref:learn:security/native-encryption-at-rest-overview.adoc[].

Encryption at rest is supported by memory-optimized index storage, standard index storage, and Vector Indexes.
When encryption at rest is active, the following Index Service items are encrypted:

* Index definitions
* Indexer stats and projector stats logs
* Scan backfill (temp) files
