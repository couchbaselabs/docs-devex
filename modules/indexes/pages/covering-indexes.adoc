...
For more information, see xref:manage:manage-settings/install-sample-buckets.adoc[Sample Buckets].
...
When a query uses a covering index, the EXPLAIN statement shows that a covering index is used for data access, avoiding the overhead associated with key-value document fetches.
Consider a secondary index, `idx_state`, on the attribute `state` in the `hotel` keyspace:
...
If you select `state` from the `hotel` keyspace, the actual values of the field `state` that are to be returned are present in the index `idx_state`, and avoids an additional step to fetch the data.
...
<.> The index scan step uses the index you created ...
...
<.> The `covers` object does not exist, showing that the data is not covered by the index.
<.> The index scan step uses the index you created ...
