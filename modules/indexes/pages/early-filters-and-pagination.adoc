...
If a query is not covered by an index, then the query needs to fetch data from the Data service after performing the index scan, in order to return the requested results.
...
If the Query Service has to perform a fetch from the Data service, and then apply any operations that could not be pushed down to the Index service, this still leads to some inefficient queries.

In Couchbase Server, ((early filtering)), ((early ordering)), and ((early pagination)) provide another line of defense, in cases where covering indexes and index pushdowns are not available.
...
In practice, an uncovered query may use a mixture of the methods illustrated by <<fig-uncovered>>, <<fig-pushdown>>, and <<fig-early>>.
...
<.> The filter `city = "Paris"` generates an exact span, and so can be pushed down to the Index service.
====
...
However, if you change the order of the ordering terms so that they no longer match the index key order, ordering cannot be pushed down to the Index service.
...
In this case, early ordering, limit, and offset are done by the Query Service before fetch.
...
Similarly, if you change the direction of the sort from descending to ascending, the ordering cannot be pushed down to the Index service, since it does not support reverse index scan.
...
* The query must reference a single keyspace with no joins.
...
* Only `IndexScan` is supported for early ordering.
