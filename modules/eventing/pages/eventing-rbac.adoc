...
NOTE: JWT Authentication support for Eventing is supported in the next major release of the Couchbase Server.

In addition to Basic Authentication and mutual TLS(mTLS), Eventing supports *JSON Web Token (JWT) Authentication*. 
This provides secure, short-lived sessions for crucial operations, consistent with the JWT support available for Indexing, Query, Backup, and XDCR.

JWT Authentication is performed over the REST API by passing a bearer token in the `Authorization` header. 
It operates in two distinct provisioning modes: 
* `jitProvisioning=true`: The user is just-in-time (JIT) provisioned from an external Identity Provider (IdP).
* `jitProvisioning=false`: The user already exists in Couchbase RBAC.

[NOTE]
====
When JWT Authentication is active with `jitProvisioning=true`, guardrails in the Eventing codebase restrict certain operations. Specifically, *creating, importing, or deleting* Eventing Functions via the REST API is not supported.
All other Eventing REST API operations remain fully functional under this mode.

To create, import, or delete Eventing Functions, you must use one of the following authentication methods instead:

* Basic Authentication
* mTLS
* JWT Authentication with `jitProvisioning=false`

In all the cases, the user must be present in Couchbase RBAC with at least the <<minimal-eventing-rbac-role, Minimal Eventing RBAC role>> privileges.
====
...
* xref:learn:security/roles.adoc[Role-Based Access Control (RBAC)]
* xref:learn:security/security-overview.adoc[Security Management Overview]
//// 