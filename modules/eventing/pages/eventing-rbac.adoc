...
NOTE: JWT authentication support for Eventing as an Early Release feature in Couchbase Server 8.1.0.
...
JWT authentication is performed over the REST API by passing a bearer token in the `Authorization` header. It operates in two distinct provisioning modes: 
