...
NOTE: xref:eventing-language-constructs.adoc#bucket_accessors[Basic Keyspace Accessors] are easier and faster to use because they have a trivial API.

Advanced Keyspace Accessors use the same bucket bindings defined in the handler as Basic Keyspace Accessors. 
Advanced Keyspace Accessors expose a set of options and operators that can be used to:
...
The GET operation lets you read a document with metadata from the bucket.
It also allows any subsequent operations to use CAS and check or modify the expiration date in the document.
...
You can use this optional parameter to load near static data from the Data Service where every document mutation needs external data to drive the business logic of Eventing Functions.
...
The INSERT operation lets you create a new document in the bucket.
It also lets you to specify an expiration date (or TTL) in the document.

The operation fails if the document with the specified key already exists.
...
The UPSERT operation lets you update an existing document in the bucket. 
It also lets you specify an expiration date (or TTL) in the document.

If no documents exist in the bucket, the operation creates a new document with the specified key.
...
The REPLACE operation lets you replace an existing document in the bucket.
It also lets you specify the following:

* An expiration date (or TTL) in the document
...
The DELETE operation lets you delete a document in the bucket specified by the key.
It also lets you specify a CAS value to be matched as a pre-condition to proceed with the operation.
...
The INCREMENT operation lets you increment the `count` field in a specified document.
...
The DECREMENT operation lets you decrement the `count` field in a specified document.
...
The TOUCH operation lets you modify the expiration time of a document without having to first access the document.
You can use this operation when an application is handling a user session but does not need to access the database.
...
function OnUpdate(doc, meta) {
    ...
    var req = {"id": meta.id, "expiry.date": expiry};
...
Eventing Functions can listen to multiple collections through
...
Must be in the JavaScript format `"keyspace": { "bucket_name": _string_, "scope_name": _string_, "collection_name": _string_ }`.

|`meta.cas`
|Type: string

(Optional) Specified the CAS value to be used as a pre-condition for the operation.
...
If the specified key is not present in the bucket, the operation fails and `key_not_found` is returned in the error object.
...
If the operation is successful, this field returns the content of the requested document.

|`result.error`
|Type: Object

If the operation fails, this field returns an error.
...
If set to true, this field indicates that the operation failed because the specified key did not exist in the bucket.
