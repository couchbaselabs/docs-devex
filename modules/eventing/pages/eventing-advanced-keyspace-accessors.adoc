...
[#advanced-subdoc-array-op-lookin]
...
`result = couchbase.lookupIn(binding, meta, subdoc_operation_array, options)`
...
By default, a LOOKUPIN operation does not use Extended Attributes (XATTRs).
When the second argument is absent or set to `{ "xattrs": false }`, the operation focuses on the fields within the document itself instead of on any user-defined XATTRs.

To search for a document's XATTRs, you must set the second argument to `{ "xattrs": true }`.
