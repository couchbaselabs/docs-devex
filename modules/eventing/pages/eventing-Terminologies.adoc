...
The entry point `OnDelete(meta,options)` passes `meta`, which contains information like the `id` of the deleted document,
and `options` containing the boolean parameter `options.expired` that indicates if the document was removed because of a deletion or an expiration.

NOTE: It is not possible to retrieve a document that has been deleted or expired.
...
Eventing JavaScript can handle integers only up to +/- 2^53^. 
