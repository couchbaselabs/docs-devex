...
Use xref:create-search-index-ui.adoc[Advanced Mode] to add additional settings and more advanced options for your Vector Search index and xref:customize-index.adoc#type-mappings[type mappings].
...
. In your document schema, select a field that contains your vector embeddings. 
+
...
Capella should automatically fill in the dimension value for your selected array field.
...
+
For more information, see xref:xref:search:quick-index-field-options.adoc#quick-field[Quick Field Type Mapping Options].
.. Select *Index*.
. Click btn:[Submit].
...
CAUTION: Some Search index features are only available in Advanced Mode. If you edit your Search index in Advanced Mode, you cannot return to Quick Mode and keep any Advanced Mode settings. 
