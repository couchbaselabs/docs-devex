...
. Under *Select Type of Index*, click *Search Vector Index*. 
. In the *Index Name* field, enter a name for the Search Vector Index.
+
[NOTE]
====
...
. Under *Index Replicas* and *Index Partitions*, choose the number of replicas or partitions you want to create for your Search Vector Index.
+
For more information about replicas and partitions, see xref:search:set-advanced-settings.adoc[].
. In the *Select Fields* box, click a vector field from your document that you want to add to your Search Vector index. 
+
...
. Configure the settings for your vector field.
For more information about any of the available settings for a vector field, see xref:search:quick-index-field-options.adoc[].
.. In the *Type* field, select *vector* or *vector_base64*, based on the format of your vector field.
.. In the *Dimension* field, enter the dimension of your vector field.
.. In the *Similarity Metric* field, choose the similarity metric you want to use to compare vectors in your Search queries to vectors in your Search Vector Index.
.. In the *Optimized For* list, choose whether your Search Vector Index should prioritize recall, latency, or memory efficiency.
.. (Optional) To boost performance through your server's GPU, click *Use GPU for indexing and searching*.
.. (Optional) In the *Searchable As* field, set a different name that you can use to search the vector field's contents in a query.
.. Click btn:[Add].
. (Optional) To add additional fields from your documents to your Search Vector Index, click the *Vector Fields* filter to show all available document fields.
.. Click a new field to add it to the index.
For more information about the configuration options for different field types, see xref:search:quick-index-field-options.adoc[].
. Click btn:[Create Index].

TIP: By default, the *Vectors* tab uses the xref:search:create-quick-index.adoc[Quick Editor] for a Search Vector Index, which hides more advanced settings.
If you want to view all available settings for a Search Vector Index from the *Vectors* tab, you can click *Enable Advanced Options* to use the standard <<search-ui,Search UI>> to create your index.

[#example]
== Example: Creating a Search Vector Index for Vector Search Query Examples 

If you want to use the sample dataset for the examples in xref:run-vector-search-ui.adoc[] and xref:run-vector-search-sdk.adoc[], then you can xref:search:import-search-index.adoc[import] the following Search index definition into the {page-ui-name}:
...
Importing an index definition is only supported from the *Search* tab. 
...
If you used the <<search-ui,Search UI>>, a basic Search Vector Index includes the vector embeddings from the child field you specified in your type mapping.
