...
For examples specific to hybrid searches, which include xref:search:run-searches.adoc[standard Search queries] and Vector Search, see xref:search:score-fusion/run-hybrid-search-score-fusion.adoc[].
...
You can xref:search:create-child-field.adoc[create a child field] or xref:search:create-quick-index.adoc[use the Quick Index editor] to update your Search Vector Index to include the `description` field with your search results.

For example, you could use the following JSON Search Vector Index payload to create your Search index.
It includes 2 child field mappings, `colorvect_l2` and `embedding_vector_dot` on 2 different vector fields in the keyspace's documents.
...
If you did not get the search results you were expecting, you can try to recreate your Search Vector Index xref:create-vector-search-index-rest-api.adoc[with the REST API].

Search Vector Indexes can use the same settings and features as regular Search indexes.
