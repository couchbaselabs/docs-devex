...
You can use these new Search Vector Indexes for Retrieval Augmented Generation (RAG) with an existing Large Language Model (LLM). 
...
From Couchbase Server version 8.5 and later, Search Vector Indexes include new optimization features to improve performance with large numbers of vectors.
For more information, see xref:vector-search:vector-search-index-architecture.adoc#fast-merge-gpus-ivf[Fast Merge and GPU Processing for IVF Indexes].
...
You can include features from a xref:search:search.adoc[standard Search index] in your Search Vector Indexes. 
Use a Search index with Search Vector Index and standard Search features to run hybrid searches, and get more precise search results. 

As of Couchbase Server version 8.5, you can also choose the specific fusion algorithm the Search Service uses to combine scoring for standard Search and Vector Search queries.

For more information about hybrid search fusion algorithms, see xref:search:score-fusion/about-score-fusion.adoc[].

== See Also

* xref:search:search.adoc[]
* xref:search:score-fusion/about-score-fusion.adoc[]
