...
Vector Search builds on Couchbase Server's xref:search:search.adoc[Search Service] to provide vector index support.
...
For more information about all frameworks and integrations supported by Vector Search and Couchbase Server, see xref:develop:integrations.adoc[].

== Using Vector Search Indexes

You can get started using Vector Search in Couchbase Server 7.6 and later by: 

. Using a Couchbase Server database to store the data you want to use to provide context to a LLM. 
