...
You can choose to prioritize recall, latency, or memory efficiency with the xref:search:child-field-options-reference.adoc#optimized[Optimized For] setting on your index, or choose to use binary quantized indexes to reduce your memory and storage requirements.
...
[cols="1,1,2,4"]
|====
| Vector Count | Index Types | Description | Quantization

| >=10,000 
| IVF with scalar quantization
| Vectors are indexed with <<ivf,>>.
a| IVF indexes can use <<scalar-quant,>>.

If xref:search:child-field-options-reference.adoc#optimized[Optimized For] is set to: 

* *recall* or *latency*: Vector Search uses <<scalar-quant,8bit scalar quantization>>.
* *memory-efficient*: Vector Search uses <<scalar-quant,4bit scalar quantization>>.
* *bivf-sq8*: If your cluster is running Couchbase Sever version 8.5 or later, Vector Search uses <<ivf-binary,binary quantization>>.
* *ivf,rabitq*: If your cluster is running Couchbase Sever version 8.5 or later, Vector Search uses <<ivf-binary,RaBitQ quantization>>.

| >=1000
| IVF with FLAT
| Vectors are indexed with <<ivf,>> combined with <<flat,>>. 
a| IVF with FLAT indexes do not use <<scalar-quant,>>.

From Couchbase Server version 8.5 and later, if xref:search:child-field-options-reference.adoc#optimized[Optimized For] is set to: 

* *bivf-flat*: Vector Search uses <<flat-binary,binary quantization>>.
* *ivf,rabitq*: Vector Search uses <<ivf-binary,RaBitQ quantization>>.

| <1000
| FLAT
| Vectors are indexed with <<flat,>>.
a| FLAT indexes do not use <<scalar-quant,>>. 

From Couchbase Server version 8.5 and later, if xref:search:child-field-options-reference.adoc#optimized[Optimized For] is set to *bivf-flat*, Vector Search uses <<flat-binary,binary quantization>>.
|====
...
[#flat-binary]
==== FLAT Indexes and Binary Quantization 

[.status]#Couchbase Server 8.5#

FLAT indexes only support threshold-based binary quantization.
From Couchbase Server version 8.5 and later, if you want to use binary quantization, the Search Service converts each element in your vectors to a bit.
This conversion reduces the overall storage footprint of your vectors.

In threshold-based binary quantization, each bit is set to either a 0 or a 1:

* If the float is greater than 0, it becomes a 1.
* If the float is less than 0, it becomes 0.

The binary representation of the floats can keep the directional similarity of vectors and keep vectors with similar meaning close together in the binary space, just as they would be in floating-point space.

The different options for binary quantization all have similar throughput, but can vary on latency and recall.
For more information about the available quantization options, see xref:search:child-field-options-reference.adoc#optimized[Optimized For].

Binary indexes use Hamming Distance to calculate your document scores in search results.
...
[#ivf-binary]
==== IVF Indexes and Binary Quantization

[.status]#Couchbase Server 8.5#

IVF indexes can support https://github.com/VectorDB-NTU/RaBitQ-Library[RaBitQ quantization^] and threshold-based quantization.
From Couchbase Server version 8.5 and later, if you want to use binary quantization, the Search Service converts each element in your vectors to a bit.
This conversion reduces the overall storage footprint of your vectors.
...
In RaBitQ quantization, each vector is rotated on a learned orthogonal projection of the entire vector set before being quantized using threshold-based quantization.
The Search Service initially uses a random matrix for the orthogonal projection, before adjusting the matrix iteratively to get an ideal spread for thresholding.
...
From Couchbase Server version 8.5 and later, if you xref:manage:manage-security/encrypt-data-at-rest.adoc[enable encryption at rest] on your cluster, the Search Service encrypts any files related to your Search Vector indexes that contain indirect or direct references to user data. 
