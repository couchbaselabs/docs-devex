...
From Couchbase Server version 8.5 and later, you can turn on *Vector Index Fast Merge* to reduce the resource consumption required for merging segments in an IVF index during reindexing and training.
...
From Couchbase Server version 8.5 and later, the Search Service can support using the available GPUs on your cluster to improve IVF index creation, training, and <<query-gpu,searching>>.
...
From Couchbase Server version 8.5 and later, if you have turned on xref:search:child-field-options-reference.adoc#gpu[using GPUs for a vector field], the Search Service can use the available GPU resources on your cluster for query processing.
