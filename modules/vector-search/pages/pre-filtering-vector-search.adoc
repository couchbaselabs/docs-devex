...
For the best results, consider using the sample Search Vector Index from xref:create-vector-search-index-ui.adoc#example[Create an Search Vector Index with the {page-ui-name}].
--
...
A pre-filter on the query will narrow the documents searched inside the Search Vector Index to documents that have a `color` field value that closely matches `navy`:
