...
You can create index replicas for a single index as follows:
...
* In the `WITH` clause, use the `nodes` and `num_replica` attributes in the `WITH` clause together.
...
You can change the index replication settings using the {index-storage-mode}[UI], the {index-storage-settings-via-cli}[command line], or the {index-settings-via-rest}[REST API].
