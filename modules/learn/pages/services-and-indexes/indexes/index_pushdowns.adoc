...
****
The examples in this topic use the sample dataset which is supplied with Couchbase Capella.
...
.Index Projection
====
This example uses the `def_samples_route_route_src_dst_day` index that comes pre-installed and can be made by the statement:

.Index
[source,sqlpp]
----
CREATE INDEX def_samples_route_route_src_dst_day
...
USE INDEX (def_samples_route_route_src_dst_day)
...
For example, in the above query <<example-simple>> with a simple WHERE clause, the predicate (`sourceairport = "SFO"`) is pushed to the index `def_samples_route_route_src_dst_day` with the following single `span` and `range`.
...
This example uses the `def_samples_landmark_city` index that can be made by the statement:
...
This example uses the `def_samples_landmark_primary` index that can be made by the statement:

.Index
[source,sqlpp]
----
CREATE PRIMARY INDEX `def_samples_landmark_primary`
  ON landmark;
----
...
This example uses the `def_samples_landmark_city` index that comes pre-installed and can be made by the statement:
...
----
EXPLAIN SELECT * FROM landmark
USE INDEX(def_samples_landmark_city)
WHERE city = "San Francisco"
ORDER BY name
OFFSET  4000  LIMIT 10000;
----
...
However, the ASC order in <<example-idx-1>> will not be able to leverage the index order in the index `idx_landmark_city_desc`, nor will the DESC order in <<example-idx-3>> be able to leverage the index order in the index `def_samples_landmark_city`.
...
MAX() can be pushed to the Indexer even if the index was not created with matching index keys or sort order.
...
MIN() can be pushed to the Indexer even if the index was not created with matching index keys or sort order.
