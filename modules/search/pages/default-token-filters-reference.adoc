...
The Search Service's token filters work with xref:search/customize-index.adoc#tokenizers[tokenizers] to filter search input tokens. 

Use a token filter to filter a tokenizer's results and get better search result matches in your index. 

For more information about token filters, see xref:search/customize-index.adoc#token-filters[Token Filters].
