...
Use a <<compound-queries,compound query array>> inside the <<must,must>>, <<must_not,must_not>> and <<should,should>> objects to set the queries a document must match. 
...
Either array type lists all the queries that must have a match in a document to include the document in search results. 
...
Either array type lists all the queries that must have a match in a document to score the document higher in search results. 
...
Use compound query arrays with <<boolean-queries,boolean queries>>.
...
You can also use the <<range-query-string,Query string syntax>> for a numeric range query. 
...
You can also use the <<range-query-string,Query string syntax>> for a date range query. 
...
The query returns results such as `beach`, `beaches`, `Beatles`, and `beating`.
...
[#geopoint-query-distance]
=== Geopoint Queries - Distance/Radius-Based
...
NOTE: To use a Geopoint query, you must have a child field in your Search index with the `geopoint` type.
...
[#geopoint-query-rectangle]
=== Geopoint Queries - Rectangle-Based
...
[#geopoint-query-polygon]
=== Geopoint Queries - Polygon-Based
...
Use the GeoJSON query syntax to query a GeoJSON shape. 

The Search Service can return documents with GeoJSON data that intersects, contains, or exists within the defined GeoJSON shape:
...
NOTE: To use a GeoJSON query, you must have a child field in your Search index with the `geoshape` type.
...
|Object
|Yes
a| Contains the <<shape,shape property>> and <<relation,relation>> property. 
...
|Object
|Yes
a| Contains the <<type,type property>> and <<coordinates,coordinates property>>. 
...
|String
|Yes
a| A string that specifies the type of GeoJSON object to use in the query. 

Set to one of the GeoJSON shape types: 
...
The structure and nesting of the arrays depends on your chosen <<type,GeoJSON shape type>>.

The Search Service uses the first value in any `coordinates` array as a longitude value. 
...
|String
|Yes
a| Set how the Search Service should return a match for a GeoJSON search. 

You can use the following string values in the `relation` property:

* `intersects`: The document's geo location data must intersect the defined geometry for the query. 

* `contains`: The defined geometry for the query must be within the document's defined geoshape data. 

* `within`: The document's defined geoshape data must be within the defined geometry for the query.

NOTE: Not all `relation` property values are supported by all GeoJSON Shapes.
For example, a `LineString` query cannot use `within` for the `relation`, as geoshape data cannot be inside a line. 

|====

==== Available GeoJSON Shapes

You can search for the following GeoJSON shapes with the Search Service. 
...
|[[point]]Point 
a| Use the `point` geoshape with the `intersects` relation. 

You cannot use the `within` relation type, and `contains` will behave the same as `intersects`.

A `point` geoshape query searches for a single point that intersects or exists inside any geoshape data in a document. 
a|
.Search for geoshape data in the geojson field that intersects a defined Point
[source,json]
----
...
|[[linestring]]LineString
a| Use the `linestring` geoshape with the `contains` or `intersects` relations. 

You cannot use the `within` relation. 

A `linestring` geoshape query searches for a line defined by an array of points. 
A straight line contains only 2 points. 
If you define more than 2 points, the Search Service defines an arbitrary path between points.  
a|
.Search for geoshape data in the geojson field that intersects a defined LineString
[source,json]
----
...
|[[polygon]]Polygon
a| Use the `polygon` geoshape with any relation type. 

A `polygon` geoshape query searches for a defined polygon that intersects, contains, or exists inside any geoshape data in a document. 
a|
.Search for geoshape data in the geojson field that exists within a defined Polygon
[source,json]
----
...
|[[multipoint]]Multipoint
a| Use the `multipoint` geoshape with any relation type. 

A `multipoint` geoshape query searches for an array of points that intersects, contains, or exists inside any geoshape data in a document. 
a|
.Search for geoshape data in the geojson field that intersects 2 points 
[source,json]
----
...
|[[multilinestring]]MultiLineString
a| Use the `multilinestring` geoshape with the `intersects` or `contains` relation types. 

A `multilinestring` geoshape query searches for an array of LineStrings that intersects or exists inside any geoshape data in a document. 
a|
.Search for geoshape data in the geojson field that intersects a defined MultiLineString
[source,json]
----
...
|[[multipolygon]]MultiPolygon
a| Use the `multipolygon` geoshape with any relation type. 

A `multipolygon` geoshape query searches for an array of defined Polygons that intersects, contains, or exists inside any geoshape data in a document. 

For a `contains` match, every defined polygon must be contained within a defined geoshape in a document. 

For a `within` match, every defined polygon must collectively contain any defined geoshapes in a document.
a|
.Search for geoshape data in the geojson field that exists within 2 defined Polygons
[source,json]
----
...
|[[geometrycollection]]GeometryCollection
a| Use the `geometrycollection` geoshape with any relation type. 

A `geometrycollection` geoshape query searches for an array of Polygons, Points, or LineStrings that intersects, contains, or exists inside any geoshape data in a document. 

For a `contains` match, any of the query geoshapes must be contained within a defined geoshape in a document. 

For a `within` match, every defined polygon must collectively contain any defined geoshapes in a document.
a|
.Search for geoshape data in the geojson field that contains a LineString and a MultiPolygon
[source,json]
----
...
a|
a|
.Search for geoshape data in the geojson field that exists within a Circle with a radius of 100 miles
[source,json]
----
...
a|
a|
.Search for geoshape data in the geojson field that intersects with a bounded rectangle
[source,json]
----
...
When running a <<geopoint-query-distance,Geopoint Distance/Radius query>>, you can set the unit of distance to use when sorting documents. 
