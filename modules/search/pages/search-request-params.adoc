...
If you want to search your document metadata, you must prefix the metadata field's name with `_$xattrs`.
a| 
...
.Query String query for "sample" in an XATTRs description field
[source,json]
----
...
If you want to search for document metadata held in XATTRs, you must add `_$xattrs` to the start of your metadata field's name. 
