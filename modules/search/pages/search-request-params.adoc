...
[NOTE]
====
The vector in your Search query must match the dimension of the vectors stored in your Search index.
...
From Couchbase Server version 8.5 and later, if your Search Vector Index uses xref:vector-search:vector-search-index-architecture.adoc#index-formats[binary quantization], your query vector does not need to use binary values instead of floats.
====
