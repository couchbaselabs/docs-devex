...
NOTE: To return only results from your Vector Search query, do not include a `query` object with your Search request.
...
When running a xref:vector-search:vector-search.adoc[Vector Search] query without a `query` object, the Search Service automatically adds a `query` object with a `match_none` query object to your request.
