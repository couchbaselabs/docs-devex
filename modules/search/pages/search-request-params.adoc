...
NOTE: The value of `k` overrides any value set in the <<size-limit,size or limit property>>.
...
For more information about the dimension value, see the xref:search-index-params.adoc[dims property] or the xref:child-field-options-reference.adoc#dimension[Dimension option] in the UI. 
