...
NOTE: You must xref:enable-custom-script-queries.adoc[turn on Custom Script Query Execution] on your cluster before you can run a `custom_filter` query.
...
Unlike a <<custom-filter-query,`custom_filter` query>>, a `custom_score` query never removes documents from your search results.
Every document that matches the inner `query` object keeps its place in your results, with a new score from your UDF.

NOTE: You must xref:enable-custom-script-queries.adoc[turn on Custom Script Query Execution] on your cluster before you can run a `custom_score` query.

For example, the following `custom_score` query takes the values from the `reviews.ratings.Cleanliness` array and boosts a document's score if the average rating for `Cleanliness` is greater than `3.0`: 
