...
* `scan_plus`: The Search query executes but requires that the consistency vectors in the Search index match the consistency vectors in the Search query.
In `scan_plus`, the Search Service automatically fetches consistency vectors from the Data Service and adds them to the Search query.
You do not need to provide a `vectors` object.
...
If you use `scan_plus` consistency, you can omit the `vectors` object and let the Search Service automatically fetch and attach consistency vectors to your Search query.
