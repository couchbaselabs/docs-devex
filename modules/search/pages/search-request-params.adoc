...
From Couchbase Server version 8.1 and later, you can use a `custom_filter` query to run a JavaScript user-defined function (UDF) during query execution that adds or removes a document from your search results by returning a `boolean` value.
...
From Couchbase Server version 8.1 and later, you can use a `custom_score` query to run a JavaScript user-defined function (UDF) that calculates a new score for documents in your search results.
