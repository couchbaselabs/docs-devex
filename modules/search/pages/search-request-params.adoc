...
To return only results from your Vector Search query, you can set the `query` object to a <<match_none,match_none>> query.
...
If you're using the <<knn,knn object>>, the `match_none` object returns only matches to the Vector Search query inside the `knn` object. 
a|
