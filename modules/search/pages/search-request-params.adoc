...
|[[query]]query |Object |Regular or FTS Vector Index Search Only a|

An object that contains the properties for one of the supported query types.

The `query` object is required if you want to run a regular search, or a xref:vector-search:vector-search.adoc[FTS Vector Index Search] query.
If your database is running Couchbase Server version 7.6.2 and later, you do not need to include the `query` object for a pure Vector Search query. 
...
NOTE: If your database is running Couchbase Server version 7.6.2 and later, to return only results from your Vector Search query, do not include a `query` object with your Search request.
To run a FTS Vector Index query that uses regular Search Service parameters together with Vector Search to return results, see the <<query-object,Query Object>> section. 
