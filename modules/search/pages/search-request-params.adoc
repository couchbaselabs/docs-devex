...
|[[from-offset]]from/offset |Integer |No a|
...
Contains `\{facet-name\}` objects to define each xref:search-facets.adoc[facet] you want to return with search results. 
...
If you want to use the `score` property to use a xref:score-fusion/about-score-fusion.adoc[score fusion algorithm] with a hybrid search query, your `sort` object must sort by `-_score` (descending order).
...
|[[score]]score |String |No a|

Use the `score` property to set the specific xref:score-fusion/about-score-fusion.adoc[score fusion algorithm] you want to use when running a hybrid search, or to omit document scoring from search results.

You can set `score` to 1 of the following strings:

* `rrf`: Use the xref:score-fusion/about-score-fusion.adoc#rrf[Reciprocal Rank Fusion (RRF)] algorithm.
* `rsf`: Use the xref:score-fusion/about-score-fusion.adoc#rsf[Relative Score Fusion (RSF)] algorithm.
* `none`: Turn off document relevancy scoring in your search results.

If you do not include the `score` property in your query, by default, the Search Service returns document relevancy scoring in search results.

If you set `score` to `rrf` or `rsf`, you must include the `params` object in your query.
You must also set your <<sort_arr,sort array>> to sort by score, in descending order.
Score fusion algorithms do not support using <<search_after,search_after>> or <<search_before,search_before>>.

|params |Object |Only if `score` is `rrf` or `rsf` a|

Use the `params` object to set specific settings for your score fusion algorithm, when running a hybrid search. 

For more information about the `params` object, see <<fusion-params,>>.
...
You cannot use `search_after` if you set your <<score,score object>> to use a xref:score-fusion/about-score-fusion.adoc[score fusion algorithm] for a hybrid search.
...
You cannot use `search_before` if you set your <<score,score object>> to use a xref:score-fusion/about-score-fusion.adoc[score fusion algorithm] for a hybrid search.
...
The Search Service returns the `k` closest documents with vectors that are closest to the vector given in `vector`.
If your cluster is running Couchbase Server version 7.6.2 or later and you want to use `vector_base64`, it uses the `vector_base64` value, instead.

Before Couchbase Server version 7.6.9, the Search Service picked only the top `k` vectors.
As a result, the Search Service could return the same document multiple times if it had multiple vectors that best matched your query.
...
From Couchbase Server version 8.5 and later, you can also add a <<boolean-filter,filter>> object to add an additional query as a filter on your boolean query.
...
|Object
a| [.status]#Couchbase Server 8.5#

Add a `filter` object to add an additional query as a filter on the documents returned by a boolean query.
You can use any other type of query supported by the Search Service as the contents of `filter`.

The query in the `filter` object does not affect the final score of a document.
It only changes whether a document is added or removed from the search result set.

a|
.reviews.content must mention "location" and "nice view" but only return documents with free_breakfast as "true"
[source,json]
----
include::example$query-boolean.jsonc[tag=filter]
----
...
* <<geoshape-v2-queries,geoshape_v2 Queries>>
...
For more information about analyzers, see xref:customize-index.adoc#analyzers[Customize a Search Index with the  Web Console].
...
| String
| No
a| Set the specific date/time parser to use to interpret the date data in your `start` and `end` properties, and find matches in your documents.

If you do not specify a date/time parser, the Search Service uses the xref:default-date-time-parsers-reference.adoc#datetimeoptional[dateTimeOptional] date/time parser. 
...
include::partial$geoshape-v2-tip.adoc[]
...
From Couchbase Server version 8.5 and later, you can use a new `geoshape_v2` field data type and `geometry_v2` query object to query your geospatial data. 

The new field data type and query use a new matching algorithm for geospatial data that improves performance in geospatial queries.
For more information, see xref:search-for-geo.adoc#geoshape-v2[Cell-Based Geospatial Search Indexes].

A `geometry_v2` query object still can use the same properties as any other GeoJSON query type to search for geolocation data: 

[source,json]
----
include::example$query-geoshape-v2.jsonc[]
----

To use a `geometry_v2` query, you must have a child field in your Search index with the `geoshape_v2` type.
For more information about how to create a child field, see xref:create-child-field.adoc[] or <<child-fields,the {field-name} object>>.
...
|[[geometry-v2]]geometry_v2
|Object
|Yes
a| Contains the <<geoshape-v2-shape,shape property>> and <<geoshape-v2-relation,relation>> property. 

Defines the GeoJSON shape and how the Search Service should find a match in documents. 

IMPORTANT: You cannot query a `geoshape_v2` field with the old `geometry` object.
You must use `geometry_v2`.

|shape
|Object
|Yes
a| Contains the <<geoshape-v2-type,type property>>, <<geoshape-v2-coordinates,coordinates property>>, and the <<geoshape-v2-radius,radius property>>. 

Defines the specific GeoJSON shape to use in the query. 

|[[geoshape-v2-type]]type
|String
|Yes
a| Set the type to the type of GeoJSON shape you want to use in your query. 
...
* <<geojson-queries-multilinestring,>> (`MultiLineString`)
* <<geojson-queries-multipolygon,>> (`MultiPolygon`)
* <<geojson-queries-geometrycollection,>> (`GeometryCollection`)
* <<geojson-queries-circle,>> (`Circle`)
* <<geojson-queries-envelope,>> (`Envelope`)

|[[geoshape-v2-coordinates]]coordinates 
|Array 
|Yes
a| An array of coordinates that describes the shape in <<geoshape-v2-type,type>>.

For the best results, make sure the floating point values in your `coordinates` array are not longer than 8 decimal places.

The specific format of your `coordinates` array changes based on the GeoJSON shape you want to use:

* <<point-coordinates,Point>>
* <<linestring-coordinates,LineString>>
* <<polygon-coordinates,Polygon>>
* <<multipoint-coordinates,MultiPoint>>
* <<multilinestring-coordinates,MultiLineString>>
* <<multipolygon-coordinates,MultiPolygon>>
* <<geometrycollection-coordinates,GeometryCollection>>
* <<circle-coordinates,Circle>>
* <<envelope-coordinates,Envelope>>

|[[geoshape-v2-radius]]radius (Circle Queries Only)
|String
|For Circle Queries
a| If your query <<geoshape-v2-type,type>> is set to `Circle`, set the radius of the Circle. 
...
|[[geoshape-v2-relation]]relation
|String
|Yes
a| Set how the Search Service should return a match for your `geometry_v2` query.

The supported values for the `relation` property depend on your <<geoshape-v2-type,type>> value:

* <<point-relation,Point>>
* <<linestring-relation,LineString>>
* <<polygon-relation,Polygon>>
* <<multipoint-relation,MultiPoint>>
* <<multilinestring-relation,MultiLineString>>
* <<multipolygon-relation,MultiPolygon>>
* <<geometrycollection-relation,GeometryCollection>>
...
From Couchbase Server version 8.5 and later, you can use a `custom_filter` query to run a JavaScript user-defined function (UDF) during query execution that adds or removes a document from your search results by returning a `boolean` value.

NOTE: You must xref:custom-scripts/enable-custom-script-queries.adoc[turn on Custom Script Query Execution] on your cluster before you can run a `custom_filter` query.

For example, the following `custom_filter` query calculates the average `Cleanliness` rating from a `hotel` document's reviews.
Documents are only added to search results if this average rating is greater than or equal to `3.0`:

[source,json]
----
include::example$custom-scripts/query-custom-filter.jsonc[tag=short]
----

A `custom_filter` query can contain the following properties: 
...
|Object
|Yes
a| A `custom_filter` query object must contain a nested <<query-object,`query` object>>, which can be any other valid query - including <<boolean-queries,Boolean Queries>> and <<compound-queries,Compound Queries>>.
This nested query runs to find matching documents to pass to your user-defined function (UDF).

|source
|String
|Yes
a| The JavaScript source code for your UDF, as a single string.

Your `source` property must include a named function declaration, such as: 

----
function my_score(doc, params) { ... }
----

The function must return a Boolean value.

If the function returns `true`, the Search Service keeps the document in your search results.
If the function returns `false`, the Search Service removes the document from your search results.

|params
|Object
|No
a| A JSON object with any parameters that you want to pass to your UDF.

Access the `params` object from inside your UDF's `source` code as the function's second argument.
Access values using dot notation - for example, `params.min`.

|fields
|Array of Strings
|No
a| An array of xref:child-field-options-reference.adoc#store[stored] field names from your Search index that you want to pass to your UDF.

Access the field values from inside your UDF's `source` code through the `doc.fields` object.
Any nested object fields from your documents must use bracket notation and be surrounded by escaped quotes (`\"`), as shown in the preceding example query.

The Search Service converts stored field values to the following JavaScript types before passing them to your UDF:

* Text fields become strings.
* Numeric fields become numbers.
* Boolean fields become Booleans.
* Vector, geopoint, and geoshape fields become arrays.

You can set `fields` to `["*"]` to pass every available stored field from your Search index to your UDF, but this can negatively affect your Search performance. 

If a field is not stored in your Search index, your UDF receives the value of that field as `undefined`.

|====

[#custom-score-queries]
=== Custom Score Queries
[.status]#Couchbase Server 8.5#

From Couchbase Server version 8.5 and later, you can use a `custom_score` query to run a JavaScript user-defined function (UDF) that calculates a new score for documents in your search results.

Unlike a <<custom-filter-queries,`custom_filter` query>>, a `custom_score` query never removes documents from your search results.
Every document that matches the inner `query` object keeps its place in your results, with a new score from your UDF.

NOTE: You must xref:custom-scripts/enable-custom-script-queries.adoc[turn on Custom Script Query Execution] on your cluster before you can run a `custom_score` query.

For example, the following `custom_score` query takes the values from the `reviews.ratings.Cleanliness` array and boosts a document's score if the average rating for `Cleanliness` is greater than or equal to `3.0`: 

[source,json]
----
include::example$custom-scripts/query-custom-score.jsonc[tag=short]
----

A `custom_score` query can contain the following properties: 
...
|Object
|Yes
a| A `custom_score` query object must contain a nested <<query-object,`query` object>>, which can be any other valid query - including <<boolean-queries,Boolean Queries>> and <<compound-queries,Compound Queries>>.
This nested query runs to find matching documents to pass to your user-defined function (UDF).
...
The function must return a numeric value, such as:
...
The Search Service uses the returned value from your UDF as the document's new score in your search results.
If your UDF returns a runtime error, the Search Service uses the document's original score, instead.

You can access a document's original score value in your `source` object with `doc.score`.
...
TIP: To reduce the resource costs for custom script queries, consider using a <<custom-filter-query,`custom_filter` query>> to reduce the number of matching documents before you change your document scores with a `custom_score` query.
...
The setting has no effect if your `scoring_model` is set to `tf-idf`.

|[[partition_selection]]partition_selection |String |No a|
...
Use a `\{facet-name\}` object to create a xref:search:search-facets.adoc[search facet].
Search facets are aggregate information that you can choose to collect for a given search query.
...
Set the total number of unique terms to return with the search results for a xref:search-facets.adoc#term-facet[Term facet]. 

The Search Service orders facet categories from values with the most occurrences to the least occurrences.

If your documents include a greater number of unique terms than specified by your `size` property, the Search Service returns a count of those terms in the `other` key in your search results.
For more information, see xref:search-facets.adoc#term-facet[Term Facet].
...
|term_prefix |String |No a|

[.status]#Couchbase Server 8.5#

If you are collecting a xref:search-facets.adoc#term-facet[Term Facet] on Couchbase Server version 8.5 or later, add a `term_prefix` property to add a prefix filter to the facets collected by your search.

For example, you could add a facet to collect values from a `country` field, but add a filter to only return countries that start with the string `united`.

This filter does not affect document scoring.

For more information, see xref:search-facets.adoc#filter-term-facets[Filter a Term Facet].

|term_pattern |String |No a|

[.status]#Couchbase Server 8.5#

If you are collecting a xref:search-facets.adoc#term-facet[Term Facet] on Couchbase Server version 8.5 or later, add a `term_pattern` property to add a regular expression filter to the facets collected by your search.

For example, you could add a facet to collect values from an `email` field, but add a filter to only return email addresses with a `.edu` domain, by adding a regular expression filter.
...
The objects in a `numeric_ranges` array specify the range or ranges for a xref:search-facets.adoc#numeric-range-facet[Numeric Range facet]. 
...
The objects in a `date_ranges` array specify the range or ranges for a xref:search-facets.adoc#date-range-facet[Date Range facet]. 
...
You can also set a specific xref:customize-index.adoc#date-time[date/time parser] to use with the date/time data in your `start` and `end` values. 
...
|datetime_parser |String |No a|

Set the specific date/time parser to use to interpret the date data in your `start` and `end` properties, and find matches in your documents.
...
NOTE: If you want to use a xref:score-fusion/about-score-fusion.adoc[score fusion algorithm] in your <<score,`score` object>> and run a hybrid search query, the `sort` object in your query must sort your results by their scores, in descending order (`-_score`).
...
The Search Service sorts documents based on their <<score,score>> value, in ascending order. 
...
[#fusion-params]
== Score Fusion params Object

Use the `params` object with the <<score,score property>> to configure your xref:score-fusion/about-score-fusion.adoc[score fusion algorithm] when running a hybrid search with the Search Service.

The following `params` object configures the rank constant for the xref:score-fusion/about-score-fusion.adoc#rrf[Reciprocal Rank Fusion (RRF)] algorithm and sets a window size of `10`:

[source,json]
----
include::example$run-search-full-request.jsonc[tag=fusion_params]
----
...
|[[score-rank-constant]]score_rank_constant |Integer |Only if <<score,score>> is `rrf` a|

If you want to use the xref:score-fusion/about-score-fusion.adoc#rrf[Reciprocal Rank Fusion (RRF)] algorithm for your hybrid search, set `score_rank_constant` to control the rank constant value for your final score calculations.

The rank constant controls how harshly a document is penalized for its rank in your RRF calculation. 
A lower `score_rank_constant` will be more likely to rank any documents that land in the top scoring positions of your standard search or Vector Search.

For example, if: 

* Document A ranked #1 for your standard search, but did not rank for your Vector Search.
* Document B ranked #10 for both your standard and Vector Search. 

If you used a low `score_rank_constant`, such as a value between `0` and `20`, for the preceding results, Document A should rank higher than Document B. 
If you used a higher `score_rank_constant`, such as a value between `60` and `100`, Document B should rank higher than Document A, due to the document having a consensus, or ranking on both searches.

The recommended default value for your `score_rank_constant` to get the best balance is `60`.

|[[score-window-size]]score_window_size |Integer |No a|

Set the `score_window_size` to control the number of results that should be used in your score fusion calculation for your hybrid search query. 

The `score_window_size` sets the final number of results returned for your query.
A higher `score_window_size` increases the performance cost of your query, but increases the relevant hits you can return in your application.

To set the `score_window_size`: 

* Choose a value greater than or equal to <<size-limit,your query's `size` property>>. 
* Choose a value greater than or equal to `1`.

If `score_window_size` is greater than the <<k,`k` value>> in your Vector Search query, the query only returns `k` results.
If `score_window_size` is less than the `k` value, `score_window_size` truncates your results.

When paginating results from a hybrid query, make sure that the value in your query's <<from-offset,`from` property>> is always less than your `score_window_size`, to make sure the Search Service returns results.
|====
