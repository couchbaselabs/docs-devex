...
From Couchbase Server version 7.6.2 and later, you do not need to include the `query` object for a pure Vector Search query. 
...
NOTE: From Couchbase Server version 7.6.2 and later, to return only results from your Vector Search query, do not include a `query` object with your Search request.
...
From Couchbase Server version 7.6.2 and later, when running a xref:vector-search:vector-search.adoc[Vector Search] query without a `query` object, the Search Service automatically adds a `query` object with a `match_none` query object to your request.
