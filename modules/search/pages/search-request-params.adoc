...
NOTE: To use a GeoJSON query, you must have a child field in your Search index with the `geopoint` type.
For more information about how to create a child field, see xref:create-child-field.adoc[] or <<child-fields,the {field-name} object>>. 
...
|Yes
a| An array of floating point value coordinates that define the GeoJSON shape. 
...
* `contains`: The defined geometry for the query must contain the document's geo location data. 

* `within`: 

//What's the difference between contains and within?
|====
