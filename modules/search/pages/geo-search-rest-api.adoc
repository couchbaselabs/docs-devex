...
:description: Search for geospatial data in your Couchbase {page-product-name} cluster with a compatible Search index, the REST API and curl/HTTP.
:page-toclevels: 3
...
[source,json]
----
include::example$geo_search/geospatial-search-index.jsonc[]
----
...
[source,console]
----
include::example$geo_search/geospatial-search-query.sh[]
---- 
...
[source,json]
----
include::example$geo_search/geospatial-search-response.json[]
----
...
[source,console]
----
include::example$geo_search/geospatial-search-query-geojson.sh[]
---- 

If the REST API call is successful, the Search Service returns a `200 OK`. 
Using the `landmark` collection from the `travel-sample` dataset, the query can return the following JSON response: 

[source,json]
----
include::example$geo_search/geojson-search-response.json[]
----

[#geoshape-v2]
=== Example: geoshape_v2 Query
...
From Couchbase Server version 8.5 and later, if you have a `geoshape_v2` field in your Search index, you can use the xref:search-request-params.adoc#geometry-v2[`geometry_v2`] object in your query to run a geospatial query.
...
For example, the following query searches a `geoshape_v2` field, `geojson_v2`, for any locations within the defined xref:search-request-params.adoc#geojson-queries-polygon[Polygon GeoJSON Query]:

[source,console]
----
include::example$geo_search/geoshape-v2-search-query-api.sh[]
----
...
[source,json]
----
include::example$geo_search/geoshape-v2-search-response.json[]
----
