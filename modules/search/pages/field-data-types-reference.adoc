...
* A string, as 2 numeric values separated by a comma. 
* A string, as a geohash point. 
* An array, as 2 floating point integers. 
* A JSON object, with the properties `lon`/`lng` and `lat`.

For the best results with geospatial search, consider using [[geoshape-v2,geoshape_v2]], instead.
For more information about geospatial search, see xref:search-for-geo.adoc[].
...
* A `coordinates` array of floating point numbers, for each latitude and longitude coordinate point in the GeoJSON shape.
For a `geoshape_v2` GeoJSON object, these floating points should not be more than 8 decimal places.
