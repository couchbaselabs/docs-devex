...
* (Couchbase Server version 8.0 and later) xref:synonyms/synonyms-search.adoc#run-synonym-search[Synonyms and synonym searches] in your Search index.
* (Couchbase Server version 8.5 and later) xref:custom-scripts/custom-script-queries.adoc[Custom script queries].
====
...
==== Hybrid Searches with bm25

The calculation when running a hybrid Search query, that includes xref:vector-search:vector-search.adoc[a Vector Search query], for the `bm25` algorithm is still: 
...
For more information about how to page through large result sets, see xref:manage-search-pagination.adoc[].
...
* xref:manage-search-pagination.adoc[]
