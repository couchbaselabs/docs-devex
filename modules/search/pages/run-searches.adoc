...
When running a FTS Vector Index Search with the <<ui,{page-ui-name}>> or <<api,REST API>>, the Search Service displays results as a disjunct (`OR`) between your regular Search and Vector Search queries. 

TIP: When running a FTS Vector Index Search query, you should add a `boost` value to your regular Search query to level the `tf-idf` score with the knn distance.
...
If your database is running Couchbase Server version 7.6.2 and later, the {page-ui-name} lets you generate a command-line curl example or edit the JSON for your query using a built-in code editor.
...
When using {sqlpp} with a xref:vector-search:vector-search.adoc[FTS Vector Index Search] query, you have more flexibility in how you choose to display your search results. 
When running a FTS Vector Index Search with the <<ui,{page-ui-name}>> or <<api,REST API>>, the Search Service displays results as a disjunct (`OR`) between your 2 search queries. 
For example: 
...
{sqlpp} allows you to choose whether to return search results as a conjunct (`AND`) or a disjunct (`OR`) between for FTS Vector Index Search queries.
