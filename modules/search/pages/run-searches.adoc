...
You can choose a specific <<scoring,scoring algorithm>> or use xref:score-fusion/about-score-fusion.adoc[a score fusion algorithm] to change how document scores are calculated.
...
==== Hybrid Searches with tf-idf

When running a hybrid Search query with the `tf-idf` algorithm, you should add a `boost` value to your regular Search query to level the `tf-idf` score with the knn distance.
...
For more information about how to set a `boost` value, see xref:search-request-params.adoc#boost[`boost`].

If you're using the xref:score-fusion/about-score-fusion.adoc#rrf[Reciprocal Rank Fusion (RRF) algorithm] for your query, your `boost` values might not have any effect on your search results, unless they move a document across a rank boundary.

If you use the xref:score-fusion/about-score-fusion.adoc#rsf[Relative Score Fusion (RSF) algorithm], boosting might have more of an effect on search results.
The final result is dependent on how your document scores are distributed.
A large `boost` value can improve the score of 1 document, but also compress the normalized scores of other documents for that query. 
...
[#hybrid]
=== Hybrid Searches with a Search Vector Index

For a hybrid Search query, that includes xref:vector-search:vector-search.adoc[a Vector Search query] alongside a regular xref:search-request-params.adoc#query[query object], the Search Service uses the following calculation for document scores using either <<tf-idf,>> or <<bm25,>>: 
...
When running a hybrid search, the Search Service displays results as a disjunct (`OR`) between your regular Search and Vector Search queries. 

The `bm25` scoring algorithm supports better hybrid search results and richer result rankings. 
...
You can use the xref:score-fusion/about-score-fusion.adoc#rrf[Reciprocal Rank Fusion (RRF) algorithm] or the xref:score-fusion/about-score-fusion.adoc#rsf[Relative Score Fusion (RSF) algorithm] with `bm25`.
`boost` values have the same impact on `bm25` Search indexes and score fusion queries as with <<tf-idf>>:

* If you use the xref:score-fusion/about-score-fusion.adoc#rrf[Reciprocal Rank Fusion (RRF) algorithm] for your query, your `boost` values might not have any effect on your search results, unless they move a document across a rank boundary.
* If you use the xref:score-fusion/about-score-fusion.adoc#rsf[Relative Score Fusion (RSF) algorithm] for your query, the result of your `boost` depends on your document score distributions.
A large `boost` value can improve the score of 1 document, but also compress the normalized scores of other documents for that query.

From Couchbase Server version 8.1 and later, if you use xref:vector-search:vector-search-index-architecture.adoc#index-formats[binary quantization] for the vectors in your Search Vector Index, the vectors in your Search query's xref:search-request-params.adoc#knn[knn object] do not need to be in binary format.
The Search Service converts the vectors for comparison automatically.
...
* xref:score-fusion/about-score-fusion.adoc[]