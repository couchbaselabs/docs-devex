...
If your Search index definition contains a `nested` object and you choose to return that field in your search results, the response returned by the REST API to your query changes. 
...
[source,console]
----
include::example$nested/run-search-payload-nested.sh[]
----

Since the query requested the contents of all available fields in the Search index, the Search Service returns the original field content in the `fields` object. 
Any object mappings defined as `nested` appear inside a `_$nested` object. 

For example, the following is how the `fields` object appears for `hotel_28955` in search results, from the example Search index definition and query:

[source,json]
----
include::example$nested/run-search-response-nested.jsonc[tag=fields_object]
----

TIP: Click btn:[View on Github] to view the entire response.
...
If your cluster is running Couchbase Server version 7.6.2 and later, you can also copy a full command-line curl example from the {page-ui-name} to use with the REST API, or edit a generated Search query with the built-in code editor.
