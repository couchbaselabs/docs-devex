...
[#delete]
=== Delete a Child Field Mapping

To delete an existing child field mapping configuration from your Search index:

include::partial$delete-mapping-steps.adoc[]

The Search Service removes any configuration changes for that child field mapping from your Search index.
If the mapping existed as a child mapping underneath a xref:about-mappings.adoc#dynamic[dynamic mapping], the field might still exist in your Search index with different settings.
