...
|[[date-time]]Default Date/Time Parser a|

Change the default date/time parser used for date data in your documents.

For more information about the available default date/time parsers, see xref:default-date-time-parsers-reference.adoc[].

For more information about how to create your own custom date/time parser, see xref:create-custom-date-time-parser.adoc[].
...
|Store Dynamic Fields |Select *Store Dynamic Fields* to include field values in search results from a xref:about-mappings.adoc#dynamic[dynamic type mapping] in the index.

|Index Dynamic Fields |Select *Index Dynamic Fields* to include fields from a xref:about-mappings.adoc#dynamic[dynamic type mapping] in the index. 

|DocValues for Dynamic Fields |Select *DocValues for Dynamic Fields* to include the values of each field from a xref:about-mappings.adoc#dynamic[dynamic type mapping] in the index for xref:search-request-params.adoc#facets[Facets] and sorting search results.

|[[fast-merge]]Vector Index Fast Merge a|

If you're creating a xref:vector-search:vector-search.adoc[Search Vector Index] with more than 1000 vectors, select *Vector Index Fast Merge* to improve reindexing and training performance for `vector` type fields.
...
You cannot turn on this setting and also use xref:vector-search:vector-search-index-architecture.adoc#fast-merge-gpus-ivf[GPU processing] with your Vector Search index.
