...
+
Otherwise, your child mapping is a xref:about-mappings.adoc#dynamic[dynamic mapping] that includes all child fields of the JSON object. 
...
. (Optional) xref:synonyms:add-synonym-source.adoc[Add a Synonym Source].
. (Optional) To set this mapping as a xref:create-nested-object.adoc[nested object] in your documents, select *nested*.
. Click btn:[OK].

[#delete]
=== Delete a Child Mapping

To delete an existing child mapping configuration from your Search index:

include::partial$delete-mapping-steps.adoc[]

The Search Service removes any configuration changes for that mapping from your Search index.
If the mapping existed as a child mapping underneath a xref:about-mappings.adoc#dynamic[dynamic mapping], the mapping might still exist in your Search index with different settings.
