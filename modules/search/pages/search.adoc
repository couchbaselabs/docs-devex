...
:page-aliases: fts:fts-introduction.adoc, fts:full-text-intro.adoc
:description: Use the Search Service to create a customizable search experience for your cluster and your end-user applications. 
...
The search service is implemented using https://github.com/blevesearch[Bleve], an open-source indexing and search framework,
developed and maintained by Couchbase.

The search engine ingests data from the nodes within a cluster
and builds an index as a list of word tokens.
The indexed tokens are stored with metadata used to link the tokens to the original documents.
The tokens are stored in an inverted index data structure,
whereby each token is associated with a list of documents where the token can be found.

[mermaid]
.Search Engine Architecture
----
flowchart LR
    jsons@{ shape: docs, label: "JSON documents" }

    subgraph engine [Search Engine]
        analyzer@{shape: process, label: "Text Analyzer"}
        index@{shape: flip-tri, label: "Inverted\nindex"}
        database@{shape: lin-cyl, label: "Databases"}
    end
...
A Search index tells the Search Service what content to use from the documents in your cluster for processing <<queries,Search queries>>. 
...
Vector Search builds on Couchbase {page-product-name}'s Search Service to provide vector index support for Retrieval Augmented Generation (RAG) with an existing Large Language Model (LLM).
