...
:description: Use the Search Service to create a customizable search experience for your operational cluster and your end-user applications. 
...
A Search index tells the Search Service what content to use from the documents in your operational cluster for processing <<queries,Search queries>>. 
A Search index can use any field across multiple collections in a single scope.
If your operational cluster is running Couchbase Server version 7.6.2 and later, it can also include any document metadata stored in Extended Attributes (XATTRs).
...
Vector Search adds a new index type to the Search Service to support AI application development, known as an Search Vector Index.
