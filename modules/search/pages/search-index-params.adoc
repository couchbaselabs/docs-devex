...
If you use an existing UUID, the Search Service updates the existing Search index. Don't include the `uuid` property when you want to copy an index to a different cluster or create a new index.
...
Don't include the `sourceUUID` property when you want to copy an index to a different cluster, or create a new index.
...
Set a xref:customize-search-index.adoc#type-identifiers[type identifier] for the Search index to filter documents from search results: 
