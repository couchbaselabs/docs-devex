...
|name |String |Yes |The name of the Search index.  Currently names are global across a cluster, in the future we will support "scoped indexes" where names are uniqe within a scope.
...
The Search Service automatically generates a UUID for a Search index.  If this UUID exists and a create index operation is done the operation is an index update.  When copying an index to a different cluster or modifying and existing index definition to make a new index you should ensure that this UID is removed from the JSON payload.
...
The Search Service automatically finds the UUID for the bucket.  When copying an index to a different cluster or modifying and existing index definition to make a new index you should ensure that this UID is removed from the JSON payload.
...
|indexPartitions |Number |Yes |The number of partitions to split the Search index into, across the nodes you have available in your database with the Search Service enabled. This setting is useful to increase both indexing and query performance on large datasets.
...
The number of replicas you can create depends on the number of nodes you have available with the Search Service enabled. Each replica creates a full copy of the index to increase the level of high-availability.
...
For more information, see <<doc-config,Doc_config Object>>.

|mapping |Object |Yes |An object that sets the analyzers and type mappings for a Search index. 
For more information, see <<mapping,Mapping Object>>.
