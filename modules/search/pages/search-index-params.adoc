...
When you xref:search/create-search-index-rest-api.adoc[create a Search index with the REST API], you need to add a JSON payload with the settings for your index.
...
The `mapping` object contains a search index's analyzers and other xref:search/set-advanced-settings.adoc[advanced settings from the UI].
...
If the tokenizer's `type` value is `exception`, give a xref:search/customize-index.adoc#tokenizers[default tokenizer] to apply to the tokens created with the `exceptions` array.
...
For more information on the available field data types, see xref:search/field-data-types-reference.adoc[].
