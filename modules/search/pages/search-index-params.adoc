...
If you wanted to index a xref:vector-search:vector-search.adoc[vector field and turn your Search index into a Search Vector index], the `fields` array for a vector field named `embedding` could look like the following: 

[source,json]
----
include::example$search-index-fields-array-vector.jsonc[]
----
...
|[[gpu]]gpu |Boolean |No a|

include::partial$vector-search-field-descriptions.adoc[tag=gpu]
...
|vector_index_fast_merge |Boolean |No a|

If you're creating a xref:vector-search:vector-search.adoc[Search Vector Index] with more than 1000 vectors, set `vector_index_fast_merge` to `true` to improve reindexing and training performance for `vector` type fields.

This setting has no effect for smaller Search Vector indexes.

For more information, see xref:vector-search:vector-search-index-architecture.adoc#ivf[Inverted File Indexes (IVF)].

NOTE: This setting can only be turned on if you already have vectors stored in your documents.
You cannot turn on this setting if you're creating an index without any data.
