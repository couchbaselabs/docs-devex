...
For more information about analyzers, see xref:customize-index.adoc#analyzers.adoc[Customize a Search Index with the Web Console]
...
Choose a synonym source to use for synonym searches with this field.
...
For more information about how to create synonym documents, see xref:synonyms/create-synonym-collection-docs-quick.adoc[].

For more information about how to add a new synonym source for your child fields, see xref:synonyms/add-synonym-source-quick.adoc[].
...
|[[gpu]]Use GPU for indexing and searching (Vector Fields Only) a|

include::partial$vector-search-field-descriptions.adoc[tag=gpu]

|[[nested]]Treat each object in the array as a nested sub-document (Arrays of Objects Only) a|

To preserve the correct relationship between properties in nested objects when indexing an array of objects, select *Treat each object in the array as a nested sub-document*. 

To keep the default flattening behavior for arrays of objects, clear *Treat each object in the array as a nested sub-document*. 

For more information about nesting and mapping arrays of objects in the Search Service, see xref:create-nested-object.adoc[].
