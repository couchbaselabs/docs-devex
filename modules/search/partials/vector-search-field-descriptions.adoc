...
* *ivf,rabitq*: From Couchbase Server version 8.1 and later, choose this option to use https://github.com/VectorDB-NTU/RaBitQ-Library[RaBitQ quantization^] with an inverted file index.
+
This option provides better recall, but can increase latency and requires a quantization-aware embedding model.
+
For more information about this quantization option, see xref:vector-search:vector-search-index-architecture.adoc#index-formats[Vector Search and FAISS].

* *bivf-flat*: From Couchbase Server version 8.1 and later, choose this option to use binary quantization with a FLAT index.
+
This option provides better recall and latency, but can increase the storage and memory required for your index compared to other binary options.
It also requires that your vectors were generated with a quantization-aware embedding model.
+
For more information about this quantization option, see xref:vector-search:vector-search-index-architecture.adoc#index-formats[Vector Search and FAISS].

* *bivf-sq8*: From Couchbase Server version 8.1 and later, choose this option to use binary quantization with an SQ8 index.
+
This option provides a similar storage and memory requirement for your index compared to other Search Vector Index options, but can increase latency compared to *bivf-flat* or *ivf,rabitq*.
It also requires that your vectors were generated with a quantization-aware embedding model.
+
For more information about this quantization option, see xref:vector-search:vector-search-index-architecture.adoc#index-formats[Vector Search and FAISS].

For more information about Search Vector Indexes, see xref:vector-search:vector-search.adoc[] or xref:vector-search:create-vector-search-index-ui.adoc[].
// end::optimized_for[]
...
From Couchbase Server version 7.6.2 and later, Search Vector Indexes can support arrays with up to 4096 elements.
