...
For more information about FTS Vector Indexes, see xref:vector-search:vector-search.adoc[] or xref:vector-search:create-vector-search-index-ui.adoc[].
// end::optimized_for[]
// tag::similarity_metric[]
For a `vector` child field, choose the method to calculate the similarity between the vector embedding in an FTS Vector Index and the vector embedding in a Vector Search query.
...
From Couchbase Server version 7.6.2 and later, FTS Vector Indexes can support arrays with up to 4096 elements.
