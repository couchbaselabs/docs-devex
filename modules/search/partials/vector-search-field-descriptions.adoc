...
From Couchbase Server version 8.1 and later, for a `vector` child field, choose whether the Search Service should use your cluster's GPU or GPUs for indexing, training, and query processing.

Your documents must contain enough vectors and your cluster must meet the xref:vector-search:vector-search-index-architecture.adoc#fast-merge-gpus-ivf[system requirements for using GPUs for vectors].

You cannot turn on this setting if you have already turned on xref:set-advanced-settings.adoc#fast-merge[Vector Index Fast Merge] for your index.

For more information about using GPUs with a Search Vector Index, see xref:vector-search:vector-search-index-architecture.adoc#fast-merge-gpus-ivf[Fast Merge and GPU Processing for IVF Indexes].
// end::gpu