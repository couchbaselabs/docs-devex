...
====
For example, `default:{backtick}<bucket>{backtick}.samples.airline.{backtick}idx-name{backtick}` indicates the `idx-name` index on the `airline` collection in the `sample` scope in the bucket.
====
...
====
For example, `default:{backtick}<bucket>{backtick}.def_type` indicates the `def_type` index on the default collection in the default scope in the `default:{backtick}<bucket>{backtick}` bucket.
====
...
====
For example, `def_type ON default:{backtick}<bucket>{backtick}` indicates the `def_type` index on the default collection in the default scope in the bucket.

Similarly, `{backtick}idx-name{backtick} ON default:{backtick}<bucket>{backtick}.samples.airline` indicates the `idx-name` index on the `airline` collection in the `samples` scope in the bucket.
====
...
====
Create a cluster of 3 nodes and then go to menu:Settings[Sample buckets] to install the `<bucket>` bucket.
