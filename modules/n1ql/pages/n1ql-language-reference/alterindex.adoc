...
====
For example, `default:{backtick}travel-db-abc123{backtick}.samples.airline.{backtick}idx-name{backtick}` indicates the `idx-name` index on the `airline` collection in the `samples` scope in the `travel-db-abc123` bucket.
====
...
====
For example, `def_type ON default:{backtick}travel-db-abc123{backtick}` indicates the `def_type` index on the default collection in the default scope in the `travel-db-abc123` bucket.

Similarly, `{backtick}idx-name{backtick} ON default:{backtick}travel-db-abc123{backtick}.samples.airline` indicates the `idx-name` index on the `airline` collection in the `samples` scope in the `travel-db-abc123` bucket.
====
