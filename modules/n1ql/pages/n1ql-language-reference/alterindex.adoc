...
IMPORTANT: You can't use this feature with a Capella Serverless database. Custom index placement and replication is only available in Capella Provisioned or Couchbase Server Enterprise Edition databases. 
...
====
For example, `default:{backtick}$DATABASE_NAME{backtick}.samples.airline.{backtick}idx-name{backtick}` indicates the `idx-name` index on the `airline` collection in the `sample` scope in the bucket.
====
...
====
For example, `default:{backtick}$DATABASE_NAME{backtick}.def_type` indicates the `def_type` index on the default collection in the default scope in the `default:{backtick}$DATABASE_NAME{backtick}` bucket.
====
...
====
For example, `def_type ON default:{backtick}$DATABASE_NAME{backtick}` indicates the `def_type` index on the default collection in the default scope in the bucket.

Similarly, `{backtick}idx-name{backtick} ON default:{backtick}$DATABASE_NAME{backtick}.samples.airline` indicates the `idx-name` index on the `airline` collection in the `samples` scope in the bucket.
====
...
.Move the `def_samples_airport_airportname_city_faa` index from one node to another
====
Create a cluster of 3 nodes and then go to menu:Settings[Sample buckets] to install the `$DATABASE_NAME` bucket.
The indexes are then installed in a round-robin fashion and distributed over the 3 nodes.


Then move the `def_samples_airport_airportname_city_faa` index from its original node (192.168.10.*10* in this example) to a new node (192.168.10.*11* in this example).
