...
:indexing-and-query-perf: xref:indexes:indexing-and-query-perf.adoc
...
* {merge}[MERGE] queries
* {using-ai}[USING AI] queries (available in Couchbase Server 8.0 and later)
...
The statement consists of the `ADVISE` keyword, and optionally the `INDEX` keyword, followed by the query for which you want index advice -- a {select}[SELECT] query, an {update}[UPDATE] query, a {delete}[DELETE] query, a {merge}[MERGE] query, or a {using-ai}[USING AI] query.

NOTE: You can use `ADVISE` with `USING AI` only in Couchbase Server 8.0 and later.
...
In Couchbase Capella, the index advisor initially makes use of the {cbo}[cost-based optimizer].
...
* The xref:clusters:query-service/query-workbench.adoc#index-advisor[Index Advisor] in the Query tab
