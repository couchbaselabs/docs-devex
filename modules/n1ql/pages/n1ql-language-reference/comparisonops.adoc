...
| LIKE
| Match string against a character pattern expression. 

The expression can contain the following wildcards:

* Percent sign (%): Matches zero or more characters.
* Underscore (_): Matches a single character at that specific position in the string. 

To match literal instances of the wildcard characters, use an escape character.
The default escape character is the backslash (\).

To define a new escape character, use the `ESCAPE` clause. 
For example, to use the hash sign (\#) as an escape character, write: `LIKE "abc#%def" ESCAPE "#"`. 
This pattern matches the string `"abc%def"`.

You can also escape the escape character itself. 

NOTE: An empty pattern matches only an empty string. 
To match any string (including an empty string), use the pattern %. 
...
SELECT "hello world" LIKE "h_llo%" AS match_1,
       "hello world" LIKE "h%world" AS match_2,
       "hello world" LIKE "h%z%" AS match_3,
       "hello% world" LIKE "hello#% world" ESCAPE "#" AS match_4;
----
