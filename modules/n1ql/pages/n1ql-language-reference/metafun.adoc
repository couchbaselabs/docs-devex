...
This function generates an array of arrays of {startsb}``field_name``, ``value``{endsb} pairs of all possible fields in the given JSON object `obj`.
...
Place this filter last in the list or use it as the only filter to ensure the function applies all other rules.
