...
Extracts xref:n1ql:n1ql-language-reference/statements.adoc#ddl[Data Definition Language (DDL)] statements such as CREATE BUCKET or CREATE INDEX from the specified buckets.

The function returns an array of strings, with each string containing an extracted DDL statement.
...
filter:: String value LIKE pattern for matching against bucket names.  An empty string matches all names.
...
* `bucket` - Includes CREATE BUCKET statements
* `scope` - Includes CREATE SCOPE statements
* `collection` - Includes CREATE COLLECTION statements
* `index` - Includes CREATE INDEX statements
* `sequence` - Includes CREATE SEQUENCE statements
...
An array of strings, each containing a DDL statement.

=== Examples

[[extract-ddl-ex1,EXTRACTDDL() Example 1]]
.Extract all DDL statements for all buckets
====
.Query
[source,sqlpp]
----
