...
By defualt, the function masks only to alphanumeric characters while preserving whitespaces and special characters (such as `@`, `#`).
To redact all characters in a value, including symbols and spaces and special characters, set `"strict": true` in the filter object.
See <<redact-ex6>>.
