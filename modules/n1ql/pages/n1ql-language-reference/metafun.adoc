...
You can use this function to retrieve the data models of existing buckets for purposes such as replication, backup, or auditing.

The function extracts the following DDL statements:
...
Use an empty string (`""`) to match all bucket names.
...
| **flags** +
__optional__
| Specifies which types of DDL statements to extract. 
...
To extract multiple statement types, specify either an array with the string values or a numeric value representing the sum of their corresponding numeric values.

For example, to extract CREATE SCOPE and CREATE INDEX statements, input the value as `10` (`2` + `8`) or as an array of strings `["scope", "index"]`.
...
.Using string values to extract CREATE INDEX statements from the `travel-sample` bucket
====
.Query
[source,sqlpp]
----
...
----
In this query, the value `3` represents the combined flags for bucket (`1`) and scope (`2`).
