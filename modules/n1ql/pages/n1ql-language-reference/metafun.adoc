...
This function extracts Data Definition Language (DDL) statements from one or more specified buckets.
It returns an array of strings, where each string contains an extracted DDL statement.

The function supports the following DDL statements:
...
NOTE: To execute this function, you must have the `query_system_catalog` role and permissions to access each individual bucket.

=== Arguments

filter:: [Required] 
A string pattern to match against bucket names using the LIKE operator. 
An empty string (`""`) matches all bucket names.

options:: [Optional]
A JSON object specifying options for the function.
If you do not specify this argument, the output includes all supported DDL statements.
...
The value can be a numeric bit flag or an array of strings.

* `"bucket"` or `1`: For CREATE BUCKET statements
* `"scope"` or `2`: For CREATE SCOPE statements
* `"collection"` or `4`: For CREATE COLLECTION statements
* `"index"` or `8`: For CREATE INDEX statements
* `"sequence"` or `16`: For CREATE SEQUENCE statements

You cannot use string and numeric values in the same function call.
If you're using numeric flags, you can combine multiple values by adding them together.

For example, to extract CREATE SCOPE and CREATE INDEX statements, specify the value as `10` (2 + 8) or as an array of strings `["scope", "index"]`.
...
.Extract index statements from the `travel-sample` bucket using string flags
====
.Query
[source,sqlpp]
----
...
.Extract index statements from the `travel-sample` bucket using numeric flags
====
.Query
[source,sqlpp]
----
...
.Extract bucket and scope statements from the `travel-sample` bucket using combined numeric flags
====
.Query
[source,sqlpp]
----
...
----
In this query, the value `3` is the sum of the flags for bucket (`1`) and scope (`2`).
