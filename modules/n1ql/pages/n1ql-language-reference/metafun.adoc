...
statement:: A string containing the statement to evaluate.
...
[[redact,REDACT()]]
== REDACT (`expr` [ , `filter-obj1`, `filter-obj2`, ... ] )

=== Description

Redacts field names or values in a JSON object based on specified filtering criteria.
The function can either mask values with a specified character or omit fields entirely from the output. 

You can use this function to protect sensitive data such as credit card numbers or personally identifiable information (PII).

=== Arguments

expr:: [Required] An expression that resolves to a JSON object that you want to redact.

filter-obj1, filter-obj2, ...:: [Optional] One or more objects that specify specify redaction rules, such as which fields to redact, how to redact them, and whether to exclude certain fields.
+
Each filter object can contain the following fields:
+
[options="header", cols="1a,3a,1a"]
|===
| Name | Description | Schema
...
Only fields matching the pattern are redacted.

The pattern matches only single-level field names.
You cannot match nested paths.
For example, a single pattern like `"parent.child"` cannot match the nested field  
`{ "parent": { "child": "value" } }`.

If you do not provide a pattern, the filter applies to all fields.
| String

| *regex* +
__optional__
| Indicates whether the pattern is a regular expression or a simple string match.

If `true`, the pattern is treated as a regular expression.
If `false`, the pattern is treated as a simple string match.
...
| *exact* +
__optional__
| Indicates whether the pattern must match the entire field name or just a portion of it. 
...
| *name* +
__optional__
| Indicates whether to redact field names in addition to field values. 

If `true`, both field names and values are redacted.
If `false`, only field values are redacted.
...
| *strict* +
__optional__
| Indicates whether to apply strict value redaction.

If `true`, the redaction does not preserve whitespace or formatting in string values.
The entire value will be replaces with the mask, without maintaining the original structure.

If `false`, the redaction preserves whitespace and formatting in string values.
...
| *omit* +
__optional__
| Indicates whether to omit fields entirely from the output.

If `true`, matching fields are removed entirely.
...
| *mask* +
__optional__
| The string used to replace redacted fields/values.
...
| *fixedlength* +
__optional__
| Indicates whether to replace string values with a single instance of the mask. 

If `true`, an entire string is replaced with one instance of the mask.
If `false`, each character in the string is masked individually, preserving the original length of the string.

This option does not apply to fields identified as valid dates.
...
| *exclude* +
__optional__
| Indicates whether to invert the filter logic.

If `true`, fields matching the pattern are excluded from redaction; while fields that do not match the pattern are redacted. 
When you use this option, any subsequent filters are ignored.
To ensure all other rules are applied, place this as the last filter in the list or use it as the only filter.

If `false`, fields matching the pattern are redacted. 
...
If you do not provide any filter-object, the function applies a default redaction behavior, which is to redact only the values of the fields in the JSON object. This means:

The field names will remain visible.
Only the values of the fields will be replaced with a default mask (e.g., "x").

By default, the REDACT() function only redacts values of the fields, leaving the field names visible.
To redact field names as well, you need to include the name: true option in the filter-object.

When multiple filter-object arguments are provided to the REDACT() function, they are evaluated in the order they are specified.
For each field in the JSON object, the function checks the filters one by one. As soon as it finds a filter that applies to a specific field, it uses that filter and ignores the rest.
This means that the order of the filters matters. If you want a specific filter to take precedence, you should place it earlier in the list of filters.

If a JSON object contains nested objects, the redaction behavior for the nested fields will follow the filter applied to the parent field by default.
However, if there is a specific filter that directly applies to a nested field, it will override the parent field's filter.

=== Return Value

A JSON object with the specified fields redacted.
