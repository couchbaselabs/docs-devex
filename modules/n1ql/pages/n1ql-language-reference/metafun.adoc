...
Specifies all references from a query.

Can be spelled as FORMALIZE or FORMALISE.
...
[[formalize-ex,FORMALIZE() Example]]
====
.Query
[source,sqlpp]
----
SELECT FORMALIZE("select * from c1","default: example")
----
.Results
[source,json]
----
[
  {
    "$1": "select self.* from `default`:` example`.`_default`.`c1`"
  }
]
----
====
