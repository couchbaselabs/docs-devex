...
+
For example, when you call a global function `default:global()` which contains the keyspace reference `{backtick}my-db{backtick}`, the keyspace reference is always resolved within the context of the function to the `default:{backtick}my-db{backtick}` bucket.
...
+
For example, when you call a scoped function `default:{backtick}my-db{backtick}.samples.scope()` which contains the keyspace reference `route`, the keyspace reference is always resolved within the context of the function to `default:{backtick}my-db{backtick}.samples.route`.
...
Similarly, to create a scoped function in a particular scope, the function name must be a qualified identifier with the full path to a scope, such as `default:{backtick}my-db{backtick}.samples.func1`.
...
For example, to refer to a scoped library called `my-library` located in the `samples` scope within the `my-db` bucket, you would specify the library name as `my-db/samples/my-library`.
