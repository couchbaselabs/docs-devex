...
+
For example, when you call a global function `default:global()` which contains the keyspace reference `{backtick}travel-db-abc-123{backtick}`, the keyspace reference is always resolved within the context of the function to the `default:{backtick}travel-db-abc-123{backtick}` bucket.
...
+
For example, when you call a scoped function `default:{backtick}travel-db-abc-123{backtick}.samples.scope()` which contains the keyspace reference `route`, the keyspace reference is always resolved within the context of the function to `default:{backtick}travel-db-abc-123{backtick}.samples.route`.
...
Similarly, to create a scoped function in a particular scope, the function name must be a qualified identifier with the full path to a scope, such as `default:{backtick}travel-db-abc-123{backtick}.samples.func1`.
...
// FIXME:
External functions in {sqlpp} _only_ support plain JavaScript, without any of the xref:server:eventing:eventing-language-constructs.adoc#added-lang-features[added language features] supported by functions in the Eventing Service.
...
For example, to refer to a scoped library called `my-library` located in the `samples` scope within the `travel-db-abc-123` bucket, you would specify the library name as `travel-db-abc-123/samples/my-library`.
