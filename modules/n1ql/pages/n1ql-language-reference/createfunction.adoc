...
+
For example, when you call a global function `default:global()` which contains the keyspace reference `{backtick}$DATABASE_NAME{backtick}`, the keyspace reference is always resolved within the context of the function to the `default:{backtick}$DATABASE_NAME{backtick}` bucket.
...
+
For example, when you call a scoped function `default:{backtick}$DATABASE_NAME{backtick}.samples.scope()` which contains the keyspace reference `route`, the keyspace reference is always resolved within the context of the function to `default:{backtick}$DATABASE_NAME{backtick}.samples.route`.
...
Similarly, to create a scoped function in a particular scope, the function name must be a qualified identifier with the full path to a scope, such as `default:{backtick}$DATABASE_NAME{backtick}.samples.func1`.
...
For example, to refer to a scoped library called `my-library` located in the `samples` scope within the `$DATABASE_NAME` bucket, you would specify the library name as `$DATABASE_NAME/samples/my-library`.
