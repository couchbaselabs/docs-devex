...
* If an external function defines an alias for an N1QL() call, EXPLAIN FUNCTION cannot return its line number.
...
[source,ebnf]
----
include::partial$grammar/utility.ebnf[tag=explainfunction]
----

image::n1ql-language-reference/explainfunction.png["Syntax diagram: refer to source code listing", align=left]
...
<.> An N1QL() call that executes a {sqlpp} statement.
...
<.> The line number in the JavaScript function that includes an N1QL() call. 
