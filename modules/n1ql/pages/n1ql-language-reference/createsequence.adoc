...
:description: The CREATE SEQUENCE statement enables you to create a sequence in a given keyspace.
...
include::n1ql-language-reference/sequenceops.adoc[tags=overview;attributes]
...
When you back up or restore a bucket, sequences are included automatically.
...
The CREATE INDEX statement provides two possible syntaxes for specifying options for a sequence.
...
If a sequence with the same name already exists within the specified keyspace, then:
...
Use the INCREMENT BY clause to specify the incremental value of each step in the sequence.

If this clause is omitted, the incremental value is `1`: that is, at each step in the sequence, the value goes up by `1`.
...
Use NO MINVALUE to specify that the minimum value is the lowest signed 64-bit integer, `-2^63^-1`.
...
Alternatively, use the WITH clause to specify options for the sequence using a JSON object.
