...
TIP: You can also use negative hints that direct the optimizer not to use certain indexes or join methods.
For more information, see xref:n1ql-language-reference/negative-keyspace-hints.adoc[Negative Keyspace Hints].
...
When you apply INDEX_ALL on a keyspace, the optimizer performs an intersect scan across all specified indexes.
...
The INDEX_ALL hint operates on an all-or-nothing basis.
...
The hint is invalid with zero or one index.

* You can only use GSI indexes with this hint.
It does not support FTS indexes. 
...
* INDEX_COMBINE is an alias for INDEX_ALL and you can use it interchangeably.
====
...
The hint is invalid if there's zero or one index.
...
Use this object to specify indexes for a single keyspace.
It must contain a <<keyspace-property-all>> and an <<indexes-property-all>>.
The order of the properties within the object is not significant.
...
NOTE: You can use this hint only in xref:n1ql-language-reference/selectclause.adoc[SELECT] and xref:n1ql-language-reference/merge.adoc[MERGE] statements. 
...
* xref:n1ql-language-reference/negative-hints.adoc[]