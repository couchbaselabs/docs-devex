...
If you examine the plan for this query, you can see that the optimizer has selected the index `def_samples_route_sourceairport`, which is installed with the sample dataset.
...
.INDEX hint
====
The following query hints that the optimizer should select the index `def_samples_route_route_src_dst_day` for the keyspace `route`.
