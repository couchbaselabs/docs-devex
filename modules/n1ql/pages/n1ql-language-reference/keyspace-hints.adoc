...
TIP: While most keyspace hints guide the optimizer to use certain indexes or join methods, you can also use negative hints to avoid methods that you don't want the optimizer to use. 
...
[NOTE]
====
* This hint is only supported in xref:n1ql-language-reference/selectclause.adoc[SELECT] and xref:n1ql-language-reference/merge.adoc[MERGE] statements.
