...
TIP: While most keyspace hints guide the optimizer to use certain indexes or join methods, you can also use negative hints to prevent the optimizer from using methods you want to avoid.
...
NOTE: You must specify at least two indexes. 
...
[NOTE]
====
* Currently, this hint is only supported in xref:n1ql-language-reference/selectclause.adoc[SELECT] and xref:n1ql-language-reference/merge.adoc[MERGE] statements.
...
* xref:n1ql-language-reference/negative-keyspace-hints.adoc[]