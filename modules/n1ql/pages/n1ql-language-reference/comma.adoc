...
Compare the query above with the following query using the optional comma-separated LATERAL join.

.Comma-separated LATERAL join
[source,sqlpp]
----
SELECT * FROM [{"abc": 1}, {"abc": 2}, {"abc": 3}] AS a, LATERAL
              [{"xyz": 1}, {"xyz": 2}] AS b;
----

Compare the queries above with the following query using an ANSI join.
...
The results of the three queries are the same.
