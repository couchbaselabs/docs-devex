...
NOTE: You cannot specify a hint for the same keyspace using both the `USE` clause and an <<hint-comment,optimizer hint>>. 
...
[[ex-delete-opt-hint]]
.Delete query with an optimizer hint
====
...
[source,sqlpp]
----
include::example$dml/delete-optimizer-hint.n1ql[]
----
...
----
include::example$dml/delete-optimizer-hint.jsonc[tag=index]
----
====

[[ex-delete-use-clause]]
.Delete query with a USE INDEX clause
====

The following query hints the Query Service to use the index, `def_inventory_hotel_city`. 
This is equivalent to <<ex-delete-opt-hint>> but uses a `USE INDEX` clause instead of an optimizer hint.

[source,sqlpp]
----
include::example$dml/delete-use-index.n1ql[]
----
...
[source,json,indent=0]
----
include::example$dml/delete-use-index.jsonc[tag=index]
----
====