...
NOTE: You cannot specify a hint for the same keyspace using both the USE KEYS clause and a <<hint-comment,hint comment>>. 
If you do this, the USE KEYS clause and the <<hint-comment,hint comment>> are both marked as erroneous and ignored by the optimizer.
