...
[IMPORTANT]
====
Alternatively, you can specify named parameters and positional parameters using the {sqlpp} REST API (`/query/service` endpoint), or a software development kit (SDK).
...
When you specify parameters with the USING clause, you cannot also specify parameters at the same time using the {sqlpp} REST API, or an SDK.
