...
[IMPORTANT]
====
Alternatively, you can specify named parameters and positional parameters using the Data API (Query Service passthrough), the `cbq` command line tool, or a software development kit (SDK).
...
When you specify parameters with the USING clause, you cannot also specify parameters at the same time using the Data API, the `cbq` command line tool, or an SDK.
...
.Executing a prepared statement without a name using the Data API
====
...
.Executing a prepared statement with a name using the Data API
====
...
* For information on using prepared statements with the Data API (Query Service passthrough), refer to xref:data-api-guide:data-api-intro.adoc[].
