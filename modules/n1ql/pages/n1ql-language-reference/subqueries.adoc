...
[source,sqlpp]
----
SELECT count(*) FROM `<BUCKET_NAME>`.samples.airport t
LET x = t.geo
...
For example, the following example <<Q6,Q6>> has no explicit alias for ``<BUCKET_NAME>``.
...
[
  {
    "code": 3000,
    "msg": "Ambiguous reference to field <BUCKET_NAME>.",
    "query_from_user": "SELECT t1.city, t1.geo.alt\nFROM `<BUCKET_NAME>` t1\nWHERE t1.type = \"airport\" AND \n(SELECT RAW t2.alt \n FROM `<BUCKET_NAME>`.geo t2)[0] > 4000;"
  }
]
----
