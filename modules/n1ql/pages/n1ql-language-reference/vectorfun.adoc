...
== DECODE_VECTOR(`vector` [,{nbsp}``byte_order`` [,{nbsp}``sparse``]])
...
If the `"sparse"` argument is present, the string must represent the base64 encoding of a sparse-term weight array.
...
sparse:: [Optional] A string with the value `"sparse"`.
If present, the `vector` argument must be a base64 encoded sparse term-weight array.

=== Return Value

An array of floating point numbers, or a sparse term-weight array, in which the first item is an array of integers representing terms, and the second item is an array of floating point numbers representing the corresponding term weights

=== Examples

[#decode_vector_ex]
.DECODE_VECTOR() Example 1
====
...
[#decode_vector_ex_sparse]
.DECODE_VECTOR() Example 2
====
The following query decodes the base64 encoding of a sparse term-weight array using two different byte orders.

.Query
[source,sqlpp]
----
SELECT DECODE_VECTOR("AQAAAAIAAAAAAEBAAACAQA==", false, "sparse") AS little_endian,
       DECODE_VECTOR("AAAAAQAAAAJAQAAAQIAAAA==", true, "sparse") AS big_endian;
----
...
== ENCODE_VECTOR(`vector` [,{nbsp}``byte_order`` [,{nbsp}``sparse``]])
...
If the `"sparse"` argument is present, this must be a sparse-term weight array, or any {sqlpp} expression that evaluates to a sparse term-weight array.
...
sparse:: [Optional] A string with the value `"sparse"`.
If present, the `vector` argument must resolve to a sparse term-weight array, in which the first item is an array of integers representing terms, and the second item is an array of floating point numbers representing the corresponding term weights.
...
[#encode_vector_ex_sparse]
.ENCODE_VECTOR() Example 2
====
The following query encodes a sparse term-weight array using two different byte orders.

.Query
[source,sqlpp]
----
SELECT ENCODE_VECTOR([[1, 2], [3, 4]], false, "sparse") AS little_endian,
       ENCODE_VECTOR([[1, 2], [3, 4]], true, "sparse") AS big_endian;
----
...
[
  {
    "little_endian": "AQAAAAIAAAAAAEBAAACAQA==",
    "big_endian": "AAAAAQAAAAJAQAAAQIAAAA=="
  }
]
----
====
...
| Determines whether to return the individual input scores alongside the final fused score.
...
[[sparse_vector_distance,SPARSE_VECTOR_DISTANCE()]]
== SPARSE_VECTOR_DISTANCE(`vec`, `queryvec` [,{nbsp}``nprobes`` [,{nbsp}``topNScan``]])

[.status]#Couchbase Server 8.5#

This function has an alias <<aliases,SPARSE_DISTANCE()>>.

=== Description

Finds the distance between a provided array of sparse term-weight pairs, and the content of a specified field that contains sparse term-weight pairs.

This function works best with a sparse Hyperscale Vector index or a sparse Composite Vector index.
If a query contains this function, and the cluster has a Hyperscale Vector index or a Composite Vector index with a sparse vector index key which is the same as the sparse vector field referenced by the function, then the Query optimizer selects that Hyperscale Vector index or Composite Vector index for use with the query containing this function.

This function always uses xref:vector-index:vectors-and-indexes-overview.adoc#dot[Dot Product] as the distance metric comparing the sparse term-weight pairs.

=== Arguments

vec:: The name of a field that contains sparse term-weight pairs.
The field must contain a sparse term-weight array, or a string representing a base64 encoded sparse term-weight array, in which the first item is an array of integers representing terms, and the second item is an array of floating point numbers representing the corresponding term weights.

queryvec:: A sparse term-weight array, or a string representing a base64 encoded sparse term-weight array, in which the first item is an array of integers representing terms, and the second item is an array of floating point numbers representing the corresponding term weights.
This represents the sparse term-weights to search for in the vector field.

nprobes:: [Optional] An integer representing the number of centroids to probe for matching vectors.
If the Query Service selects a Hyperscale Vector index or Composite Vector index for the query, this option defaults to the `scan_nprobes` setting that you used when you created the index.
If an invalid value is provided, defaults to `1`.

topNScan:: [Optional; can only be used when `nprobes` is specified]
This option only applies if using a Hyperscale Vector index.
A positive integer representing the number of records to scan.
The default is `0`, meaning the function uses the indexer default.
...
Returns a negative numeric value representing the vector distance.

=== Examples

To try the examples in this section, you must do the following:

. Download the example file xref:vector-index:attachment$rgb-sparse.json[rgb-sparse.json], and import the data into a collection called `pass:c[`vector-sample`.color.`rgb-sparse`]`.

. Download the example file xref:vector-index:attachment$vocabulary.json[vocabulary.json], and import the data into a collection called `pass:c[`vector-sample`.color.vocabulary]`.

. Create a sparse Hyperscale Vector index in the `rbg-sparse` collection on the field named `sparse_vectors`, as described in xref:n1ql:n1ql-language-reference/createvectorindex.adoc#ex-create-sparse-idx[CREATE VECTOR INDEX Example 5].

[#sparse_vector_distance_ex_simple]
.SPARSE_VECTOR_DISTANCE() Example 1
====
The following query finds the vector distance between sets of sparse term-weight pairs.
...
WITH data AS ([
  {"vector": [[1, 2], [3, 4]], "similarity": "identical"},
  {"vector": [[1, 2], [3, 5]], "similarity": "close"},
  {"vector": [[1, 2, 6], [7, 8, 9]], "similarity": "distant"}
])
SELECT
  similarity,
  SPARSE_VECTOR_DISTANCE(vector, [[1], [3]]) AS one_term,
  SPARSE_VECTOR_DISTANCE(vector, [[1, 2], [3, 4]]) AS two_terms
FROM data;
----
...
[#sparse_vector_distance_ex_simple]
.SPARSE_VECTOR_DISTANCE() Example 2
====
...
The query returns 6 colors where the sparse term-weight pairs associated with the color name and description match via dot-product overlap with the query term and weight.
...
The query only returns 6 results because only these results 
====


[#sparse_vector_distance_ex_multiple]
.SPARSE_VECTOR_DISTANCE() Example 3
====
The following query finds colors from the `rgb-sparse` collection whose titles or descriptions contain exact or near-exact matches for the terms `light`, `sea`, and `green`.

.Query
[source,sqlpp]
----
include::vector-index:example$hyperscale-idx-examples.sqlpp[tag=sparse-query-multiple]
----

. The ARRAY constructor uses a subquery to find the integer associated with each of the query terms in turn. @/vocabulary/

. The ARRAY_FLATTEN function flattens the results to generate the first element in the query term-weights array.
This two-step process ensures that the query terms are preserved in the correct order. @/ARRAY_FLATTEN/

. The second element in the array assigns the same weight to each of the query terms. @/1.0/

. The `sparse_vector` field in the `rgb-sparse` collection contains sparse term-weight pairs based on the color names and descriptions. @/rgb-sparse/

The query returns 10 colors where the sparse term-weight pairs associated with the color name and description match via dot-product overlap with the query terms and weights.

.Results
[source,json]
----
include::vector-index:example$hyperscale-idx-data.json[tag=sparse-query-multiple]
----
====
...
* `SPARSE_VECTOR()` is an alias for <<sparse_vector_distance>>.
