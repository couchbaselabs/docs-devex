...
Vector functions include similarity functions to find the distance between two vectors, functions that check for a vector value, and functions that transform vector values.
...
* There is a hyperscale vector index or a composite vector index with a vector index key which is the same as the vector field referenced by the function
...
In this case, the approximate vector distance does not give very accurate results.
====
...
For more details and examples, see xref:vector-index:hyperscale-reranking.adoc[].
====
...
This can be used to determine whether a field contains contains a vector value.
...
To normalize the vector, the function calculates the length of the vector by adding the squares of each floating point number in the array.
The function then divides each floating point number in the array by the length of the vector.
