...
* Include a [def]_relative path_, containing just the bucket and scope, followed by the collection name;
...
====
This statement deletes a collection called `city` in the `samples` scope within the bucket.

include::ROOT:partial$replace-bucket-name.adoc[]

[source,sqlpp]
----
DROP COLLECTION $DATABASE_NAME.samples.city
----
====
