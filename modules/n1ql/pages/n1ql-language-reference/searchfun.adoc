...
If your database is running Couchbase Server version 7.6.2, if there is no suitable Search index, but you want to return XATTRs data from your documents in another part of your {sqlpp} query, you must use the xref:metafun.adoc#meta[META function] to select the XATTRs field you want to return.
