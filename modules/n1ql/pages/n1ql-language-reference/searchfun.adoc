...
It is also recommended, but not required, that you should create suitable Search indexes for the searches that you need to perform.
...
[Optional]
| string, object
| The `index` field may be a string, containing the name of a Search index in the keyspace.
(This may be a Search index alias, but only if the Search index is in the same keyspace.)
...
[Optional]
| string
| A string, containing the UUID of a Search index in the keyspace.
This provides an index hint to the Query engine.
...
Click a Search index name in the Capella UI to open the editor. Click btn:[Index Definition] to view the index definition.
