...
From Couchbase Server version 7.6.2 and later, you can also use the search functions to use document metadata stored in Extended Attributes (XATTRs) inside a {sqlpp} query.
...
From Couchbase Server version 7.6.2, if there is no suitable Search index, but you want to return XATTRs data from your documents in another part of your {sqlpp} query, you must use the xref:metafun.adoc#meta[META function] to select the XATTRs field you want to return.
...
From Couchbase Server version 7.6.2, if you want to return XATTRs data from a Search index in a <<search,>> function, you must use the `SEARCH_META()` function to select the XATTRs field you want. 
