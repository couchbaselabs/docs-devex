...
Search functions enable you to use xref:search:search.adoc[full-text search queries] directly within a {sqlpp} query.
...
| string
| A query string.
For more details, refer to xref:search:search-request-params.adoc#query-string-query-syntax[Query String Query Syntax].

| object
| The query object within a full text search request.
For more details, refer to xref:search:search-request-params.adoc#query-object[Query Object].

| object
| A complete full text search request, including sort and pagination options, and so on.
For more details, refer to xref:search:search-request-params.adoc[Search Request JSON Properties].
...
[Optional]
| string
| A string, containing the xref:search:search-index-params.adoc#uuid[UUID] of a full text index in the keyspace.
This provides an index hint to the Query engine.
...
For more information on defining custom type mappings within the full text search index, refer to xref:search:create-type-mapping.adoc[Create a Type Mapping].
