...
If your database is running Couchbase Server version 7.6.2, if there is no suitable Search index, but you want to return XATTRs data from your documents in another part of your {sqlpp} query, you must use the xref:n1ql-language-reference/metafun.adoc#meta[META function] to select the XATTRs field you want to return.
...
If your database is running Couchbase Server version 7.6.2 and you want to return XATTRs data from a Search index in a <<search,>> function, you must use the `SEARCH_META()` function to select the XATTRs field you want. 
