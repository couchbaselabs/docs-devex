...
For example, you can use the CURL function with the INSERT-SELECT statement to import external data into Couchbase Capella.
...
[source,sh]
----
include::server:rest-api:example$query-settings-post-access.sh[]
----

The access list file command structure is described in the following table.

.Structure of Access List for CURL()
include::server:rest-api:partial$query-settings/definitions.adoc[tag=access]
...
FROM CURL("http://localhost:8093/query/service",
   {"data": "statement=SELECT * FROM route t
           ...
           & query_context=$DATABASE_NAME.samples",
...
RBAC credentials are required when CURL() is accessing Couchbase Capella.
...
====
The following example shows how to use the `CURL()` function to include a full text search from the xref:server:fts:full-text-intro.adoc[Search service] in a {sqlpp} query.
Assuming the FTS index `fts_travel` is created on the default collection in the default scope of the sample dataset, running the following {sqlpp} query finds all documents that have "sanfrancisco" anywhere in the document.

You can use xref:n1ql-language-reference/searchfun.adoc[search functions] to use a full text search in a {sqlpp} query, as long as the Search service is available on the cluster.
...
// TODO: completely overhaul this to use `$DATABASE_NAME`.tenants.users

Insert the following documents, representing customers and their minimum thresholds, into the default collection of the sample data scope, and then run the `SELECT` query:

.Data
[source,sqlpp]
----
INSERT INTO `$DATABASE_NAME` (KEY, VALUE)
  VALUES
     ("k1",
