...
NOTE: Optimizer hints are available only in <<ansi-merge>>.
You can use both hash and join hints, but ORDERED hints are not supported.
For an example of using an optimizer hint, see <<example-5>>.
...
In Couchbase Server 8.0 and later, you can also use optimizer hints on the merge target. 
However, you cannot specify a hint for the same keyspace using both the USE clause and an <<hint-comment, optimizer hint>>. 
...
[[example-5]]
.ANSI merge with an optimizer hint
====
...
[source,sqlpp]
----
include::example$dml/ansi-merge-hint.n1ql[]
----
...
----
include::example$dml/ansi-merge-hint.jsonc[tag=hint]
----
====
