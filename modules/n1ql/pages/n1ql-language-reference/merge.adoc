...
[IMPORTANT]
====
* Optimizer hints are only supported in <<ansi-merge>> and not in <<lookup-merge>>.
* ORDERED hints are not available in MERGE statements.
====
...
This example uses an optimizer hint to specify that the `airport` keyspace should be joined using a hash join.
...
If you examine the query plan, you will see that the optimizer has used the index specified in the hint.
