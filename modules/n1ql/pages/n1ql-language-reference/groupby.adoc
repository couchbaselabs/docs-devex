...
If the `FROM` clause of the query includes an alias or aliases, the `GROUP AS` array wraps each object in an outer object identified by its `FROM` alias. 
Identifying the objects with their aliases helps avoid ambiguity.
