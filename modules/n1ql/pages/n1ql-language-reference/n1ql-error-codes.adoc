...
You can set a memory quota with the Couchbase Capella UI, the REST API, or the CLI. For more information, see xref:n1ql:n1ql-manage/query-settings.adoc[Query Settings and Parameters].

* To set a memory quota with the UI, see xref:server:manage:manage-settings/general-settings.adoc#query-settings[Query Settings] in the General settings for Couchbase Capella.
* To set a memory quota with the REST API, see xref:n1ql:n1ql-manage/query-settings.adoc#queryMemoryQuota[queryMemoryQuota] in Query Settings and Parameters.
* To set a memory quota with the CLI, see xref:server:cli:cbcli/couchbase-cli-setting-query.adoc[setting-query] in the CLI Reference.
