...
ansi-join-lateral:: <<ansi-join-lateral>> icon:caret-down[]
...
Couchbase supports lateral queries with and without the LATERAL keyword.
This makes Couchbase queries compatible with queries from relational databases.
...
[#ANSI-Join-Example-4]
.RIGHT OUTER JOIN of <<ANSI-Join-Example-3>>
====
...
TIP: You can also supply a join hint within a specially-formatted xref:n1ql-language-reference/optimizer-hints.adoc[hint comment].
