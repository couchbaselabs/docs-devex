...
For example, in the query `SELECT {backtick}travel-db-abc123{backtick}.name FROM {backtick}travel-db-abc123{backtick}`, `{backtick}travel-db-abc123{backtick}.name` is a more formal way of expressing the identifier name.
