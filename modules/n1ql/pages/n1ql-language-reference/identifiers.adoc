...
For example, in the query `SELECT {backtick}my-db{backtick}.name FROM {backtick}my-db{backtick}`, `{backtick}my-db{backtick}.name` is a more formal way of expressing the identifier name.
