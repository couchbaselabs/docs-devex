...
For example, in the query `SELECT {backtick}$DATABASE_NAME{backtick}.name FROM {backtick}$DATABASE_NAME{backtick}`, `{backtick}$DATABASE_NAME{backtick}.name` is a more formal way of expressing the identifier name.
