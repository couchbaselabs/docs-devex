...
****
The examples on this page use the sample dataset, which needs to be installed before use.
...
To see the query execution plan, try these statements with the Query Workbench.
...
====
For this example, first insert the following documents into the default collection in the default scope in database:

NOTE: Replace $DATABASE_NAME with the name of the bucket in your database.

[source,sqlpp]
----
INSERT INTO $DATABASE_NAME VALUES ("account-customerXYZ-123456789",
{ "accountNumber": 123456789,
  "docId": "account-customerXYZ-123456789",
  "code": "001",
  "transDate":"2016-07-02" } );

INSERT INTO $DATABASE_NAME VALUES ("codes-version-9",
{ "version": 9,
...
CREATE INDEX idx_account_customer_xyz_transDate
    ON $DATABASE_NAME (SUBSTR(transDate,0,10),code)
...
FROM $DATABASE_NAME AS account
JOIN $DATABASE_NAME AS codesDoc ON KEYS "codes-version-9"
