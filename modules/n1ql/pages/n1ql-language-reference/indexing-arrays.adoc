...
.Nested array index
====
WARNING: Please note that the example below will alter the data in the sample dataset.
To restore your sample data, remove and reinstall the sample dataset.
...
For example, <<Q8D>> below uses the index `def_samples_route_route_src_dst_day` ignoring the {use-index-clause}[USE INDEX] hint to use the array indexes.
