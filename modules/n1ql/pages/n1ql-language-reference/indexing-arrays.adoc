...
.Variable Expression
====
You can index one or more expressions _within_ the array (up to maximum of 32) by using the {flatten-keys}[FLATTEN_KEYS()] function in the `var-expr`.
...
You can add the `INCLUDE MISSING` modifier to the first argument in the {flatten-keys}[FLATTEN_KEYS()] function, in order to index array elements in which the specified field is missing.
====
