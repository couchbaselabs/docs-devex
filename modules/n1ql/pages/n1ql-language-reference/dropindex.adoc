...
//====
//For example, `def_type ON default:{backtick}$DATABASE_NAME{backtick}` indicates the `def_type` index on the default collection in the default scope in the `default:{backtick}$DATABASE_NAME{backtick}` bucket.

//Similarly, `{backtick}idx-name{backtick} ON default:{backtick}$DATABASE_NAME{backtick}.samples.airline` indicates the `idx-name` index on the `airline` collection in the `samples` scope in the `default:{backtick}$DATABASE_NAME{backtick}` bucket.
//====
