...
:drop-vector-index: xref:n1ql-language-reference/dropvectorindex.adoc
...
index-path:: (Required) See <<index-path>>.
...
[source,ebnf]
----
include::partial$grammar/ddl.ebnf[tags=index-name-on-keyspace]
----
...
keyspace-ref:: (Required) See <<keyspace-ref>>.
...
When using memory-optimized indexes, this statement is an expensive operation and may take a few minutes to complete.
...
[caption=Attention]
IMPORTANT: We recommend that you do not drop (or create) secondary indexes, composite vector indexes, or hyperscale vector indexes when any Index service node is down, as this may result in duplicate index names.
//end::usage[]
