...
====
For example, `default:{backtick}$DATABASE_NAME{backtick}.samples.airline.{backtick}idx-name{backtick}` indicates the `idx-name` index on the `airline` collection in the `samples` scope in the `default:{backtick}$DATABASE_NAME{backtick}` bucket.
====
...
NOTE: Replace $DATABASE_NAME with the name of the bucket in your database.
