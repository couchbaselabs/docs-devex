...
[Optional] The SAVE keyword saves the query plan for the prepared statement to persistent storage.
Unlike plans in the <<cache, statement cache>>, saved plans persist across Couchbase Server restarts and upgrades.
For more information, see xref:n1ql:n1ql-manage/query-plan-stability.adoc[].

NOTE: The SAVE keyword is only available in Couchbase Server 8.5 and later. 
...
NOTE: Prepared statements in the cache exist only in memory and are lost after a Couchbase Server restart or upgrade.
In Couchbase Server 8.5 and later, you can save a prepared statement and its query plan in persistent storage using the <<syntax, SAVE>> keyword. 
...
In Couchbase Server 8.5 and later, if you use the <<syntax, SAVE>> keyword, the plan survives node restarts and upgrades independently of statement propagation. 
Saving the plan ensures it remains available without requiring you to re-prepare the statement.
For more information, see xref:n1ql:n1ql-manage/query-plan-stability.adoc[].
...
[[ex-prepare-save]]
.Prepared statement with SAVE keyword
====
The following example prepares a query and saves its plan to persistent storage.
