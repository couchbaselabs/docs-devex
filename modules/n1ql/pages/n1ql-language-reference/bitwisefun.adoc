...
The bitwise AND operation compares each bit of `int_value1` to the corresponding bit of every other `int_value`.
If all bits are 1, then the corresponding result bit is set to 1; otherwise it is set to 0 (zero).

=== Syntax

....
BITAND(int_value1, int_value2, ...)
....
...
compares each bit of `int_value1` to the corresponding bit of every other `int_value`.
...
This function has an alias <<aliases, ISBITSET()>>.
...
Some bitwise functions have aliases:

* `ISBITSET()` is an alias for <<section_unf_2sv_s1b>>.