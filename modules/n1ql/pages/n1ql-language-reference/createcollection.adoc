...
For example, `default:{backtick}my-db{backtick}` indicates the `my-db` keyspace in the `default` namespace.
...
====
This statement creates a collection called `city` in the `samples` scope within the `my-db` bucket.

NOTE: Replace my-db with the name of the bucket in your database.

[source,sqlpp]
----
CREATE COLLECTION `my-db`.samples.city
----
====
