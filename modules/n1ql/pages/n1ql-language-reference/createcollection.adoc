...
bucket::
(Optional) An {identifier}[identifier] that refers to the {logical-hierarchy}[bucket] in which you want to create the collection.
...
If you specify a collection name by itself without setting a valid query context, the collection is created in the default scope.
...
====
This statement creates a collection called `city` in the `samples` scope within the database.

[source,sqlpp]
----
CREATE COLLECTION samples.city
----
====
...
Assuming that the query context is set, this statement creates a collection called `country` in the `samples` scope within your bucket.
