...
NOTE: To convert a compressed string back to its original format, use the xref:n1ql:n1ql-language-reference/stringfun.adoc#fn-str-uncompress[UNCOMPRESS()] function. 

=== Arguments
input_string:: A string or a valid expression that evaluates to a string. 
...
NOTE: This function is often used together with the xref:n1ql:n1ql-language-reference/stringfun.adoc#fn-str-compress[COMPRESS()] function, which compresses and encodes strings to `base64` format. 

=== Arguments

input_string:: A `base64` encoded string that represents the compressed data. 
...
//TODO: Add link to the AWR documentation, when it is ready.  
