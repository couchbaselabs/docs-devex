= GRANT
:description: The GRANT statement allows granting any RBAC roles or credentials a user or group.
...
Using `GRANT`, you can provide two types of access:

* Roles: Grants RBAC roles to a user or group, giving them a predefined set of privileges.
* Credentials: Permits a user or group to use a specific credential from the credential store. 
To grant credential permissions, use the `GRANT CONSUME CREDENTIALSTORE` syntax. 
...
[source, sqlpp]
----
GRANT CONSUME CREDENTIALSTORE aws_key
   TO linda;
----

NOTE: Mixing of parameterized and unparameterized roles or syntax is not allowed and will create an error.

NOTE: `GRANT CONSUME CREDENTIALSTORE` grants access to a named credential in the credential store rather than an RBAC role, and cannot be mixed with role-based GRANT syntax in the same statement.
...
.Grant a specific user permission to consume the `awsid` credential
====
[source,sqlpp]
----
GRANT CONSUME CREDENTIALSTORE `awsid` TO debby;
----
====
...
[source,sqlpp]
----
GRANT CONSUME CREDENTIALSTORE `awsid`, `backup_s3` TO GROUP sales;
----
====
