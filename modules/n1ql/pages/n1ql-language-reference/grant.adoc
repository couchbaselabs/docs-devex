...
+
For example: `pass:c[DataReader ON `$DATABASE_NAME`]` +
or `pass:c[Query_Select ON `$DATABASE_NAME`]`
...
.Grant the roles of Cluster Administrator and Data Reader in the keyspace to Debby
====
...
[source,sqlpp]
----
GRANT ClusterAdmin, DataReader ON `$DATABASE_NAME` TO debby;
----
====