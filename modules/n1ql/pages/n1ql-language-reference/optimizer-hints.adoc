...
Currently, a hint comment is only supported in `SELECT`, `DELETE`, `UPDATE`, and `MERGE` statements.
...
----
EXPLAIN
UPDATE /*+ {"index": {"keyspace": "airport", "indexes": "def_inventory_airport_city"}} */ airport
SET updated = true
WHERE city = "San Francisco";
----
...
[#ex-update-hint-explain]
.Explain plan for UPDATE with a simple index hint
====
...
[#ex-delete-hint-explain]
.Explain plan for DELETE with a simple index hint
====
...
* xref:n1ql:n1ql-language-reference/negative-keyspace-hints.adoc[] apply to a specific keyspace, but instruct the optimizer not to use certain options.
