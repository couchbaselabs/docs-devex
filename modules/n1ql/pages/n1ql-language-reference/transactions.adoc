...
You can then click btn:[Run] to execute the transaction.

* To execute a single statement as a transaction, set the *Run as transaction* option in the Query Options dialog.

In either case, you do not need to specify the `txid` parameter or the `tximplicit` parameter.
If you need to specify any other parameters for the Couchbase transaction, you can use the Query Options dialog.
...
To use the examples in this section, unset the query context.
For more information, see xref:n1ql:n1ql-intro/queriesandresults.adoc#query-context[Query Context].

.Keyspace
For the purposes of this worked example, you must create a new scope containing two new collections.

====
include::ROOT:partial$query-context.adoc[tag=unset]

.Create a scope for tenants
[source,sqlpp]
----
CREATE SCOPE tenants IF NOT EXISTS;
----

.Create a collection for bookings
[source,sqlpp]
----
CREATE COLLECTION tenants.bookings IF NOT EXISTS;
----

.Create a collection for users
[source,sqlpp]
----
CREATE COLLECTION tenants.users IF NOT EXISTS;
----

.Populate the users collection
[source,sqlpp]
----
UPSERT INTO tenants.users
VALUES ("0", {"name": "Keon Hoppe"}),
       ("1", {"name": "Rigoberto Bernier"});
----
====
...
. To specify the transaction scan consistency, in the *Scan Consistency* list, select *not_bounded*.

. To specify the transaction timeout, in the *Transaction Timeout* box, enter `120`.

. In the *Named Parameters* section, click the btn:[+] button to add a named parameter.

. Enter `durability _level` in the *name* box and `"none"` (with double quotes) in the *value* box.
...
To use the examples in this section, set the query context to the `tenants` scope in the sample dataset.
For more information, see xref:n1ql:n1ql-intro/queriesandresults.adoc#query-context[Query Context].
