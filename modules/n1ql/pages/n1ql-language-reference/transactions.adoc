...
Then switch the *transaction* toggle on and click btn:[Run]

* To execute a single statement as a transaction, turn on the *transaction* toggle, enter the statement in the Query Editor and click btn:[Run].

In either case, you do not need to specify the `txid` parameter or the `tximplicit` parameter.
If you need to specify any other parameters for the Couchbase transaction, you can use the Query Options dialg.
...
To try the examples in this section, set the query context to the `tenants` scope in the sample dataset.
