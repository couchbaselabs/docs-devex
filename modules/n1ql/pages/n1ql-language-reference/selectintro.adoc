...
// Cross-references
:query-select: xref:clusters:manage-database-users.adoc#about-database-user-permissions
...
To execute the following statement, the user must have {query-select}[read] access to `airline`.
...
To execute the following statement, the user must have {query-select}[read] access to `route` and `airline`.
...
To execute the following statement, the user must have {query-select}[read] access to `airport` and `landmark`.
...
To execute the following statement, the user must have {query-select}[read] access to `hotel` and `landmark`.
...
In databases running Couchbase Server 7.6 or later, the projection is returned in the order specified by the query.
