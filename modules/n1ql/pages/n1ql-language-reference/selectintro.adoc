...
* You can add the `INCLUDE MISSING` modifier to a leading index key in the index definition, in order to index documents in which the specified key is missing. For more information, see xref:n1ql:n1ql-language-reference/createindex.adoc#include-missing[INCLUDE MISSING]
