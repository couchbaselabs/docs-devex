...
WITH RECURSIVE recroute AS (
    ...
    FROM `travel-sample`.inventory.route WHERE sourceairport="TLV"
        UNION
    SELECT r.sourceairport, rdestinationairport, recroute.depth+1 as depth
    FROM `travel-sample`.inventory.route r JOIN recroute ON recroute.destinationairport = r.sourceairport
    WHERE r.sourceairport!="TLV" and recroute.depth<2 )
SELECT * FROM recroute;
----
...
[source,sqlpp]
----
SELECT r.sourceairport, r.destinationairport, recroute.depth+1 as depth 
FROM `travel-sample`.inventory.route r JOIN recroute ON recroute.destinationairport = r.sourceairport 
WHERE r.sourceairport!="TLV" and recroute.depth<2 
----
