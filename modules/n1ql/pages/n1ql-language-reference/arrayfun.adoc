...
* `field[]`: 
** If `field` is an array, this is equivalent to `field` itself and returns the array as is.
** If `field` is not an array, it returns `NULL`.
* `field[].field2`:
** Returns an array of `field2` values from each element in the `field` array. 
** Elements in `field` that are not objects or do not contain `field2` are ignored.
* `field[].field2[].field3`: 
** Returns an array of `field3` values by traversing two levels of arrays.
** First through each element of `field`, then through each element of `field2` within those elements, extracting `field3` from each valid object. 
* `field[][].field2`: 
** Returns `MISSING` if elements of `field[]` are not arrays.

NOTE: If you use more that two empty array subscripts (for example, `field[][][]`), only the first two are considered.
Any additional subscripts are ignored.
