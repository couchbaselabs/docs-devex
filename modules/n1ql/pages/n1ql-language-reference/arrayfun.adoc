...
If the input [.var]`expr` is `MISSING` or if one of the elements in the array is `MISSING`, then it returns `MISSING`.
...
=== Return Values
A number representing the arithmetic mean (average) of all the non-`NULL` number values in the array [.var]`expr`.
...
=== Description
This functions checks if the array [.var]`expr` contains the specified [.var]`val`.
...
If the [.var]`expr` argument is not an array, it returns `NULL`.

If the array [.var]`expr` contains [.var]`val`, it returns `TRUE`; otherwise, it returns `FALSE`.
...
=== Description
This function inserts one or more specified values into a specified position in the input array [.var]`expr`, and returns the new array.
...
This function has a synonym xref:n1ql-language-reference/metafun.adoc#len[LEN()]
...
=== Description
This function returns the first position of a specified [.var]`value` within the array [.var]`expression`.
...
expr:: [Required] The array you want to have the input [.var]`value` argument(s) prepended to.
...
val1, val2, ...:: [At least 1 is required] The value or multiple value arguments that you want appended to the end of the input array [.var]`expr`.
...
Successive numbers are incremented by [.var]`step_num`.

If [.var]`step_num` is not specified, then the default value is 1.
If [.var]`step_num` is negative, then the function decrements until it reaches the smallest number greater than [.var]`end_num`.
