...
:roles: xref:organizations:organization-projects-overview.adoc
...
:query-monitoring: xref:n1ql:n1ql-manage/monitoring-n1ql-query.adoc
...
You can use monitoring catalogs to gather diagnostic information.
For example, if you are performing a bulk insert using a `SELECT` statement, you can use monitoring catalogs to get the number of documents being inserted.
...
You can configure this value using
ifdef::flag-devex-rest-api[]
the {pipeline-batch-srv}[pipeline-batch] service-level setting, or
endif::flag-devex-rest-api[]
the {pipeline_batch_req}[pipeline_batch] request-level parameter.
Note that the maximum batch size is (2^32^ -1) and specifying a value higher than this may increase the memory consumption.
ifdef::flag-devex-rest-api[]
...
====
Set the parameter `$faa_code` using the cbq prompt, or the {query-preferences}[Run-Time Preferences] in the Query tab.
