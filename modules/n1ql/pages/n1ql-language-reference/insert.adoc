...
It includes only the attributes you want to add or update, and does not replace all existing attributes. 
+
You must provide the complete value for each attribute. 
You cannot specify only the nested fields within an attribute, as the insert operation sets the full value at the top-level. 
