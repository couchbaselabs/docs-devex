...
+
The object only includes the attributes you want to add or update. It does not affect other existing attributes. 
+
You must provide the complete value for each attribute.
You cannot specify only the individual nested fields, as each attribute is updated as a whole. 
...
options::
[Optional] An object representing the metadata to be set for the inserted document.
Only the listed attributes have any effect; any other attributes are ignored.
