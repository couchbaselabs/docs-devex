...
This scope and collection are created automatically within the bucket, if they do not already exist.

=== Security for Optimizer Statistics

Access to the `_system` scope and `_query` collection is restricted to system administrators.
Regular users do not have access to this scope or collection.
Users with system administration privileges can access or drop them.
If you drop this scope or collection, all optimizer statistics stored in the collection will be lost.
A new `_system` scope and `_query` collection will be created again the next time an `UPDATE STATISTICS` statement is run.

The `_system` scope and `_query` collection are not listed in the *Explore Your Data* area in the Query Workbench.

=== Memory Quota for Optimizer Statistics

For large clusters, it is recommended that system administrators monitor the memory usage of any bucket which contains optimizer statistics.
As any bucket containing optimizer statistics grows larger, increase the memory quota for that bucket so that it does not fall under 15% memory-resident.
