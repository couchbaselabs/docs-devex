...
This option does not apply to sparse vector indexes.
|Integer
...
For sparse vector indexes, the distance metric is always `DOT`.
...
For sparse vector indexes, the description is always `IVF`.
...
[[ex-training-list-wait]]
.Create a Composite Vector Index without data
====
For this example, it's assumed that the bucket, scope, and collection exist, but no sample data is installed.
The path to the required keyspace is specified by the query, so you do not need to set the query context.

Create a Composite Vector index that indexes the vector field named `embedding-vector-dot`, like <<ex-create-vectors-idx>>.
This time, specify that the indexer should wait for the keyspace to contain sufficient data before training the index.

[source,sqlpp]
----
include::vector-index:example$gsi-vector-idx-examples.sqlpp[tag=create-wait-idx]
----

For more examples, see xref:vector-index:vector-index-without-data.adoc#examples[Examples].
====
