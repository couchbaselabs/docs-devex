...
////
In the {console-indexes}[Indexes page in the Capella UI], index replicas are marked with their replica ID.
...
You can query the {querying-indexes}[system:indexes] catalog to find the ID of an index replica and see which node it is placed on.
