...
If the `nodes` property is specified, the Index planner places the index on one of the nodes in the array.
...
If you specify the `nodes` property by itself, the index is placed on one of the destination nodes, and one replica is placed on each of the others.
...
In this case, the number of nodes in the array must be greater than the specified number of replicas, otherwise the index creation fails.
****
+
...
The value of this property must be lower than the number of index nodes in the cluster.
If `nodes` is specified, the value of this property must be lower than the number of nodes.
If the value of this property is too high, then the index creation fails.
****
