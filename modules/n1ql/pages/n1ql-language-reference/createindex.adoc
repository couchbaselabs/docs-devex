...
For more information, see <<index-with,WITH Clause>> below.
...
For more information, see {indexing-arrays}[Array Indexing].
...
NOTE: It's not recommended to create (or drop) secondary indexes when any node with a secondary index role is down, as this may result in duplicate index names.
...
For more information, see {indexing-meta-info}[Indexing Meta Info].
