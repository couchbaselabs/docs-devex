...
[source,sh]
----
curl http://$HOST:9102/settings -u $USER:$PASSWORD \
...
[source,sh]
----
curl -X GET http://$HOST:9102/settings -u $USER:$PASSWORD
----

* `$HOST` is the hostname for the API call, for example: `localhost`.
...
[[ex-create-idx-vector]]
.Create a composite vector index
====
For this example, you must install the sample data as described in {prerequisites}[Prerequisites].
