...
image::n1ql-language-reference/begin-transaction.png["Syntax diagram: see source code listing", align=left]
...
If you're using the Data API, you must set the {txid}[txid] query parameter to specify the transaction ID for any subsequent statements that form part of the same transaction.
endif::flag-devex-rest-api[]

ifdef::flag-devex-query[]
If you're using the Query tab, you do not need to specify the transaction ID for any statements that form a part of the same transaction within a multi-statement request.
...
// Similarly,
If you're using the cbq shell, you do not need to specify the transaction ID for any statements that form a part of the same transaction.
