...
====
Before executing the following statement, you must set the `natural_cred`, `natural_orgid`, and `natural_context` parameters in the xref:tools:query-workbench.adoc#query-preferences[Run-Time Preferences] window.
...
====
This is equivalent to <<example-10>>, but specifies all parameters directly in the statement, instead of setting them at the request-level.
