...
:description: The USING AI statement allows you to generate {sqlpp} queries from natural language prompts.
...
IMPORTANT: The word `AI` is recognized as a keyword, but only when used as part of the `USING AI` statement.
...
CAUTION: If you're using the USING AI statement on Couchbase Capella, we recommend that you use the cbq shell, an SDK, or the Data API.
...
image::n1ql-language-reference/using-ai.png["Syntax diagram: see source code listing", align=left]
...
+
By default, the statement uses the {natural_orgid}[natural_orgid], {natural_cred}[natural_cred], and {natural_context}[natural_context] request-level parameters to determine the organization ID, credentials, and keyspaces for the request.
...
If specified, this value overrides the {natural_cred}[natural_cred] request-level parameter.
...
If specified, this value overrides the {natural_context}[natural_context] request-level parameter.
...
If specified, this value overrides the {natural_orgid}[natural_orgid] request-level parameter.
...
// For more information about how to set request-level parameters, see {query-settings}#section_nnj_sjk_k1b[Configure Queries].
...
* To execute the statement on Capella, use the cbq shell, an SDK, or the Data API.
The statement does not function when run directly through the Capella UI (Query tab) or Couchbase Shell (cbsh).
...
* xref:data-api-reference:index.adoc[]