...
+
For example: `pass:c[DataReader ON `<BUCKET_NAME>`]` +
or `pass:c[Query_Select ON `<BUCKET_NAME>`]`
...
.Revoke the roles of ClusterAdmin and QueryUpdate in the <BUCKET_NAME> keyspace from debby
====
[source,sqlpp]
----
