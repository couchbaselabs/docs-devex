...
Consider the following indexes, which are included with the sample data that is shipped with the product.
...
[source,sqlpp]
----
CREATE INDEX idx_name ON `collection`(field1, field2, ..., field9);
----
====
+
...
+
====
This example uses the `def_samples_hotel_city` index, which is installed with the sample data.
...
. *Adaptive index requires more storage and memory*, especially in case of Memory Optimized Indexes.
 ...
 .. For example, the following queries show that a single route document in the sample dataset generates 103 index items and that all route documents produce ~2.3 million items.
+
====
[source,sqlpp]
----
...
For example, the following query uses a default index, such as [.var]`def_samples_hotel_city`, instead of the specified adaptive index [.var]`ai_city1`:
+
====
[source,sqlpp]
----
