...
:description: Keyspace hints that instruct the optimizer to not use certain indexes or join methods.
...
Currently optimizer hints  are all “positive”, i.e., instructing the optimizer to do something, e.g., use this index or this join method. Relational systems also frequently support “negative” hints, i.e., instructing the optimizer NOT to do something. We will be adding negative versions of the index hints and join hints.

For the negative optimizer hints, we support 2 versions of the syntax. One version uses the “NO_” prefix, which is for compatibility with relational databases such as Oracle and Postgres; one version uses the “AVOID_” prefix, which is used by some relational databases such as Informix.

There are no equivalent legacy hints for the negative hints, since previously Couchbase does not support negative optimizer hints. This is in contrast to the positive hints which have legacy counterparts.

== NO_INDEX or AVOID_INDEX
...
[source,sqlpp]
----
include::example$select/index-all-hint.n1ql[tag=query]
----
...
This hint directs the optimizer to consider all the specified secondary indexes. 
If not specified, the optimizer selects the optimal available index.

When you specify INDEX_ALL on a keyspace, the optimizer performs an intersect scan across all listed indexes.
It waits for all index scans to complete before intersecting the document IDs.




the optimizer uses all listed indexes to perform an intersect scan.
It waits for all scans to complete
Instead, it waits for all index scans to complete before performing a complete intersect of the document IDs.

The INDEX_All hint uses a “all or nothing” semantics.
It either uses all the indexes specified, or it does not consider the hint at all.
The optimizer does not attempt to consider a subset of the indexes specified.
If the INDEX_ALL hint is not followed, the optimizer falls back to its normal process for picking index(es) for the keyspace.

An INDEX_ALL hint is only considered “followed” if all the indexes specified are usable for the keyspace in the query.
If one or more of the indexes cannot be used (due to lack of necessary predicates on index keys), then the INDEX_ALL hint is considered “not followed”.

[NOTE]
====
* You must specify at least two indexes in the hint.
If you provide no indexes or only one index, the hint is considered invalid.

* Only supports GSI indexes.
You cannot use FTS indexes with this hint. 

* The INDEX_ALL hint cannot be used with any other index hints for the same keyspace.
It must be the only index hint present. 

* There is no negative hint for the INDEX_ALL hint.

* The INDEX_ALL hint can be used for a keyspace on the right-hand side of an ANSI JOIN or ANSI NEST.
* The INDEX_ALL hint cannot be used for a keyspace on the inner of a legacy join/nest, i.e. lookup join/nest or index join/nest
* INDEX_COMBINE is accepted as an alias for INDEX_ALL.
====
...
[source,ebnf]
----
include::partial$grammar/hints.ebnf[tag=gsi-all-hint-simple]
----

image::n1ql-language-reference/gsi-all-hint-simple.png["Syntax diagram: refer to source code listing", align=left]
...
NOTE: You must specify at least two indexes in the hint.
If you provide no indexes or only one index, the hint is considered invalid.
...
[source,ebnf]
----
include::partial$grammar/hints.ebnf[tag=gsi-all-hint-json]
----

image::n1ql-language-reference/gsi-all-hint-json.png["Syntax diagram: refer to source code listing", align=left]

With the JSON syntax, this hint takes the form of an `index_all` property.
You may only use this property once within the hint comment.
The value of this property may be an <<index-array-all>> or an <<index-object-all>>.
...
Use this array to specify indexes for multiple keyspaces.
Each element must be an <<index-object-all>>.
...
[#index-all-examples,reftext="INDEX_ALL Examples"]
=== Examples
...
.Optimized index selection
====
The following query does not include an INDEX_ALL hint.
...
.INDEX hint
====
The following query hints that the optimizer should select the index `def_inventory_route_route_src_dst_day` and `def_inventory_route_sourceairport` for the keyspace `route`.
...
[source,json,indent=0]
----
include::example$select/index-all-hint.jsonc[tag=index]
----
====
