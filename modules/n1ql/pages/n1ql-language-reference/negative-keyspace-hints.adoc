...
It directs the optimizer not to use a specific index or indexes for a keyspace.
...
[#no-index-fts-examples,reftext="NO_INDEX_FTS Examples"]
=== Examples
...
[#ex-no-index-fts-hint]
.Query Without an Index Hint
====
The following query does not include an index hint.
...
Without the NO_INDEX hint, the optimizer automatically selects the def_inventory_route_sourceairport index for the query.
...
.Query with the NO_INDEX Hint
====
The following query hints that the optimizer to avoid using the `def_inventory_route_sourceairport` index for the keyspace `route`.
...
If you examine the plan for this query, you can see that the query excludes the  `def_inventory_route_sourceairport` index, forcing the optimizer to consider other available indexes.
In this case, it has selected the `def_inventory_route_route_src_dst_day` index.
