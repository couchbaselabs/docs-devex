...
You may only use this property once within the hint comment.
The value of this property may be a <<no-hash-array>> or a <<no-hash-object>>.
...
[source,ebnf]
----
include::partial$grammar/hints.ebnf[tag=no-hash-array]
----

image::n1ql-language-reference/no-hash-array.png["Syntax diagram: refer to source code listing", align=left]

Use this array to apply the hint to multiple keyspaces.
Each element must be a <<no-hash-object>>.
...
[source,ebnf]
----
include::partial$grammar/hints.ebnf[tag=no-hash-object]
----

image::n1ql-language-reference/no-hash-object.png["Syntax diagram: refer to source code listing", align=left]

Use this object to apply the hint to a single keyspace.
It must contain a <<keyspace-property-hash>>.
The order of the properties within the object is not significant.
