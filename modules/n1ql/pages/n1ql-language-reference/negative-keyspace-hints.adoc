...
:description: Negative hints instructs the optimizer not to use certain indexes or join methods.
...
While xref:n1ql-language-reference/keyspace-hints.adoc[positive keyspace hints] force the optimizer to use a particular method, negative hints instructs it what to avoid.

Negative hints are of two types: index hints and join hints. 
Index hints direct optimizer not to use specific indexes, and join hints directs it to avoid specific join methods.
...
NOTE: Unlike xref:n1ql-language-reference/keyspace-hints.adoc[positive keyspace hints], negative hints do not have legacy equivalents. 
...
IMPORTANT: Currently, Couchbase does not support full table scans. Therefore, the optimizer ignores the NO_INDEX hint when no indexes are specified.
...
It directs the optimizer not to consider a hash join for a specified keyspace.
...
====
The following query hints the optimizer to avoid using a hash join for the `aline` keyspace. 
...
If you examine the plan for this query, you can see that the optimizer avoids using a hash join for the `aline` keyspace and considers other join methods. 
In this case, the optimizer has selected the generic join method.
...
It directs the optimizer not to consider a nested loop join for a specified keyspace.
This hint must be specified on the keyspace on the right-hand side of the join. 
If not specified, the optimizer selects the optimal join method.
