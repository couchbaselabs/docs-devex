...
While xref:n1ql-language-reference/keyspace-hints.adoc[positive keyspace hints] guide the optimizer to use specific indexes or join methods, you can use negative hints to prevent the optimizer from using methods you want to avoid.

Negative hints are of two types: index hints and join hints. 
Index hints direct optimizer not to use specific indexes, and join hints direct it to avoid specific join methods.
...
This is because Couchbase does not currently support full table scans.
...
NOTE: Currently, this hint is only supported in xref:n1ql-language-reference/selectclause.adoc[SELECT] and xref:n1ql-language-reference/merge.adoc[MERGE] statements. 
...
Also, you cannot specify a negative hint and the `USE` clause on the same keyspace.
...
As a best practice, avoid using positive and negative hints of the same type on the same keyspace, as this can lead to conflicts and cause the optimizer to ignore both hints.
