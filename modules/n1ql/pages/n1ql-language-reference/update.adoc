...
:use-clause: xref:n1ql-language-reference/hints.adoc
...
NOTE: You cannot specify a hint for the same update target using both the `USE` clause and an <<hint-comment,optimizer hint>>.
If you do this, the `USE` clause and the <<hint-comment,optimizer hint>> are both marked as erroneous and ignored by the optimizer.
...
[[example-13]]
.Update with a USE INDEX hint
====

The following query hints the Query Service to use the index `def_inventory_airport_city`.
This is equivalent to <<example-12>> but uses a `USE INDEX` clause instead of an optimizer hint.

[source,sqlpp]
----
include::example$dml/update-use-index.n1ql[]
----
...
[source,json,indent=0]
----
include::example$dml/update-use-index.jsonc[tag=index]
----
====
