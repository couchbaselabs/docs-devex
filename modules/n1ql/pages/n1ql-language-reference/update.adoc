...
:meta-xattr: xref:n1ql-language-reference/metafun.adoc#meta-xattr
...
To set extended attributes (XATTR), use {meta-xattr}[META().xattrs.<attribute>[.path\]] 
...
You cannot specify only the nested fields within an attribute as the entire top-level value is updated. 
...
The format is {meta-xattr}[META().xattrs.<attribute>[.path\]], where:
* `<attribute>` is the top-level key of the XATTR object.
* `path` is an optional subpath within the XATTR object.
