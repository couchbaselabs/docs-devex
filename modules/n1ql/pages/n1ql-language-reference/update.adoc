...
IMPORTANT: Join hints and ORDERED hints are not available in UPDATE statements.
...
====
This example uses an optimizer hint to specify that the `def_inventory_landmark_city` index should be used while updating the `nickname` attribute.
