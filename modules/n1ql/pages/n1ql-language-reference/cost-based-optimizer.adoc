...
The _cost-based optimizer_ (CBO) is a feature available in Couchbase Capella that enables the Query service to create the most efficient plan to execute a query.
...
In Couchbase Capella, two join methods are supported: nested-loop join and hash join.
