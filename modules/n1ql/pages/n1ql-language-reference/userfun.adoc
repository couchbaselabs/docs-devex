...
Similarly, to call a scoped function in a particular scope, the function name must be a qualified identifier with the full path to a scope, such as `default:{backtick}$DATABASE_NAME{backtick}.samples.func1`.
...
FROM `$DATABASE_NAME`.samples.hotel
