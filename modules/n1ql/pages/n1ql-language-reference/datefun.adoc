...
input-fmt::
A string, or any valid xref:n1ql-language-reference/index.adoc[expression] which evaluates to a string, representing a <<date-string,supported date format>> that the `date1` argument is in.
+
*Optional argument*.
Only required if the desired output format does not match the input format and `date1` isn't in a standard format.
