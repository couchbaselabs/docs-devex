// Needs editing to relate to elixir -- replicas and placement no longer relevant.
...
In Couchbase Capella, the recommended way to do this is using the `num_replicas` option.
In Couchbase Capella, you need to create multiple identical indexes and place them using the `nodes` option.
...
====
For example, `default:{backtick}my-db{backtick}` indicates the default collection in the default scope in the `my-db` bucket in the `default` namespace.

Similarly, `default:{backtick}my-db{backtick}.samples.airline` indicates the `airline` collection in the `samples` scope in the `my-db` bucket in the `default` namespace.
====
...
In Couchbase Capella, the index type for a primary index must be Global Secondary Index (GSI).
...
Create a named primary index on the default collection in the default scope within the bucket.
