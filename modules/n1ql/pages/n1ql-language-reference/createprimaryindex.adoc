...
//====
//For example, `default:{backtick}travel-db-abc123{backtick}` indicates the default collection in the default scope in the `travel-db-abc123` bucket in the `default` namespace.

//Similarly, `default:{backtick}travel-db-abc123{backtick}.samples.airline` indicates the `airline` collection in the `samples` scope in the `travel-db-abc123` bucket in the `default` namespace.
//====
