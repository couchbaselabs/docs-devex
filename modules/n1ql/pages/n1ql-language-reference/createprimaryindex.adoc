...
Primary indexes contain a full set of keys in a given keyspace.
Primary indexes are optional and are only required for running ad hoc queries on a keyspace that's not supported by a secondary index.
...
image::n1ql-language-reference/create-primary-index.png["Syntax diagram: see source code listing", align=left]
...
For more information, see {index-replication}[Index Replication].
