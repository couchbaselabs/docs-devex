...
//====
//For example, `default:{backtick}$DATABASE_NAME{backtick}` indicates the default collection in the default scope in the `$DATABASE_NAME` bucket in the `default` namespace.

//Similarly, `default:{backtick}$DATABASE_NAME{backtick}.samples.airline` indicates the `airline` collection in the `samples` scope in the `$DATABASE_NAME` bucket in the `default` namespace.
//====
