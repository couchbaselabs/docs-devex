...
//====
//For example, `default:{backtick}travel-db-abc-123{backtick}` indicates the default collection in the default scope in the `travel-db-abc-123` bucket in the `default` namespace.

//Similarly, `default:{backtick}travel-db-abc-123{backtick}.samples.airline` indicates the `airline` collection in the `samples` scope in the `travel-db-abc-123` bucket in the `default` namespace.
//====
...
[[ex-create-primary]]
.Create a primary index in the default scope
====
include::ROOT:partial$query-context.adoc[tag=unset]

Create a named primary index on the default collection `_default` in the default scope.
