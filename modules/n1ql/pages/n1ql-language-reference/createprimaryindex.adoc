...
You can handle scan timeout on a secondary index by
ifdef::flag-devex-rest-api[]
increasing the indexer timeout setting (see {query-settings}[Query Settings]), or preferably by
endif::flag-devex-rest-api[]
defining and using a more selective index.
