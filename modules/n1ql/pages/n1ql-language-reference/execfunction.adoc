...
Similarly, to execute a scoped function in a particular scope, the function name must be a qualified identifier with the full path to a scope, such as `default:{backtick}$DATABASE_NAME{backtick}.samples.func1`.
