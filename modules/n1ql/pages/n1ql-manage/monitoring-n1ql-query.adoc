...
* {sqlpp} from the cbq shell or the Query tab
...
To view system vitals, use
ifdef::flag-devex-rest-api[]
the Admin REST API or
endif::flag-devex-rest-api[]
a {sqlpp} query.
...
To terminate an active request, for instance, a non-responding or a long-running query, use
ifdef::flag-devex-rest-api[]
the Admin REST API or
endif::flag-devex-rest-api[]
a {sqlpp} query.
...
To delete a specific prepared statement, use
ifdef::flag-devex-rest-api[]
the Admin REST API or
endif::flag-devex-rest-api[]
a {sqlpp} query.
...
To get a list of all logged completed requests, use
ifdef::flag-devex-rest-api[]
the Admin REST API or
endif::flag-devex-rest-api[]
a {sqlpp} query.
...
To purge a specific completed request, use
ifdef::flag-devex-rest-api[]
the Admin REST API or
endif::flag-devex-rest-api[]
a {sqlpp} query.
...
For field names and meanings, see <<sys-active-examples>>.
...
To activate query profiling at the node level, specify the `profile` setting using the xref:n1ql-rest-admin:index.adoc[Admin REST API] (`/admin/settings` endpoint).
...
* Specify the `profile` setting using the xref:data-api-guide:data-api-intro.adoc[Data API] (Query Service passthrough).
...
The Query tab automatically enables Query profiling, with detailed timing information.
To disable or enable Query profiling with the Query tab, specify the *Collect query timings* option using the xref:clusters:query-service/query-workbench.adoc#query-settings[Query Settings].
...
include::n1ql-rest-query:index.adoc[tags=ResponseProfile]

[[ResponseProfileTiming,Execution Timings]]
*Execution Timings*

include::n1ql-rest-query:index.adoc[tags=ResponseProfileTiming]

[[ResponseProfileTimingStats,Statistics]]
*Statistics*

include::n1ql-rest-query:index.adoc[tags=ResponseProfileTimingStats]

include::n1ql-rest-query:partial$definitions/ResponseProfileTimingStats/definition-end.adoc[opts=optional]
...
* Due to the added overhead of running both profiling and xref:clusters:monitoring/monitoring.adoc#activity-logs[logging], turn on both of them only when needed.
...
For field names and meanings, see <<ResponseProfileTiming>>.
