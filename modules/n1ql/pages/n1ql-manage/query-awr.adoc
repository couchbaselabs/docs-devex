...
It is analogous to `elapsedTime` in the xref:n1ql-rest-query:index.adoc#Metrics[Query Service API] response.
...
When working with Couchbase transactions, AWR collects performance statistics for all individual statements (such as SELECT, INSERT, UPDATE), and you may notice that the COMMIT statement often shows the highest elapsed time.  
