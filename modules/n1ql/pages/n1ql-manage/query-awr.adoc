...
Each document is uniquely identified by its document key (ID), that includes the start time of the reporting interval, making it easier to filter and analyze performance data.
...
For more information about configuring the TTL, see xref:server:learn:data/expiration.adoc[Manage Expiration].
...
For more information about the repository, see <<awr-repository, AWR Repository>>.
...
It also sets the reporting interval to 10 minutes and threshold to 1 minute.
...
The documents keys (IDs) of the snapshot documents include the timestamp of the reporting interval's start time.
