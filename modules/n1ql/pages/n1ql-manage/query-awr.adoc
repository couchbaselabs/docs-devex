...
It is analogous to `elapsedTime` in the xref:n1ql-rest-query:index.adoc[Query Service API] response.
...
It is analogous to `cpuTime` in the xref:n1ql-rest-query:index.adoc[Query Service API] response when profiling is enabled.
...
A request will only return its document memory usage if memory quota was set for the query, or if node quota and node-quota-val-percent was set.

It is analogous to `usedMemory` in the xref:n1ql-rest-query:index.adoc[Query Service API] response. 
| Number
...
It is analogous to `resultCount` in the xref:n1ql-rest-query:index.adoc[Query Service API] response.
| Number
...
It is analogous to `resultSize` in the xref:n1ql-rest-query:index.adoc[Query Service API] response.
| Number
...
It is analogous to `errorCount` in the xref:n1ql-rest-query:index.adoc[Query Service API] response.
| Number


| **run time** +
| The total amount of time taken to execute the query.
| Number


| **fetch time** +
| The total amount of time spent in fetching data from the Data service. 
...
It includes the time spent executing the primary scan operator code and waiting for data from the Index service.
| Number
...
It includes the time spent executing the primary scan operator code and waiting for data from the Data service.
| Number
...
For transaction use cases, AWR collects performance statistics for all statements (such as SELECT, INSERT, UPDATE, DELETE) separately as expected. 
In the report, you may notice that COMMIT statement often shows the highest elapsed time. 
However, from the report alone, you will not be able to gather insights on why the COMMIT statement took long to execute. 
