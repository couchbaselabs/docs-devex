...
It captures key performance indicators such as query execution time, CPU usage, memory consumption, and number of executions for each query.
This information is then aggregated into <<snapshots, snapshots>> and stored in a centralized <<awr-repository, repository>>. 

To access the snapshots, you can either query the repository directly or leverage Couchbase's report generation tool to generate reports and compare query performance across different time periods.
...
The AWR repository is a centralized storage location where all snapshots are collected and maintained.
It is a user-defined location within a keyspace that can be path to a bucket or a collection, but not a scope.
...
During each reporting interval, AWR gathers and aggregates performance statistics for all queries executed on the cluster.
For every unique statement executed within that interval, AWR generates a snapshot.
The snapshot contains aggregate information for all executions of that specific statement during that period. 

Each snapshot is stored as a document in the AWR repository, uniquely identified by its document key (ID), which includes the timestamp reporting interval's start time.
...
For more information on configuring the TTL, see xref:server:learn:data/expiration.adoc[Manage Expiration].

[#enable-configure-awr]
== Enable and Configure AWR

AWR is an opt-in feature that you must explicitly enable and configure.
Once enabled, AWR starts collecting data as soon as the repository location is set and available.
...
| **location** +
| The target keyspace where the snapshots are stored. 
...
If the interval is set to 10 minutes, AWR captures a snapshot every 10 minutes. 
...
.Query to enable AWR and set the repository location
====
The following query enables AWR and sets the repository location to `default.s1.awr` with a reporting interval of 10 minutes and a threshold of 1 minute.
[source,sqlpp]
----
UPDATE system:awr SET enabled = true, location = "default.s1.awr", interval = "1m", threshold = "0s";
----
====

.Query to view the current AWR settings
====    
The following query retrieves the current AWR settings. 
[source,sqlpp]
----
...
You can access the AWR snapshot documents using two methods:

* <<report-generation-tool, By generating reports using the `cbqueryreportgen` tool>>
* <<query-awr-data-directly, By querying the AWR data directly>>

[#report-generation-tool]
=== Generate Reports using `cbqueryreportgen`

You can generate AWR reports using the `cbqueryreportgen` tool.
It provides comprehensive and user-friendly reports by executing SQL++ queries against the collected AWR snapshots.
...
For more information about the tool and its usage, see (__link to be added__).
// TODO: Add link to the CLI Reference section.  

[#query-awr-data-directly]
=== Query AWR Data Directly

You can also query AWR data directly from the AWR repository using SQL++ queries.
...
Since each snapshot is stored as a document and the document key (ID) includes the timestamp of the reporting interval's start time, you can filter documents based on time ranges without requiring indexes (as sequential scans support range-based key patterns).  
