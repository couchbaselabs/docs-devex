...
:page-aliases: manage:manage-settings/query-settings, settings:query-settings
:page-partial:
:alt-markdown-links:
...
[#service-level]
NOTE: Cluster-level settings and node-level settings are collectively referred to as service-level settings.
...
<<completed,completed>>
<<completed-stream-size,completed-stream-size>>
...
For more examples, see xref:n1ql:n1ql-manage/monitoring-n1ql-query.adoc[].
--
====
...
* You can use <<identifier,additional>> request-level parameters to supply the values for named parameters.
