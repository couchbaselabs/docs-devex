...
For example, `default:$DATABASE_NAME.samples.airline` refers to the `airline` collection in the `samples` scope in the `$DATABASE_NAME` bucket.
...
So for example, we may refer to the `$DATABASE_NAME` keyspace, meaning the default collection in the default scope in the `$DATABASE_NAME` bucket.
+
