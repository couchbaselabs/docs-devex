...
The `system:settings` catalog contains cluster-wide settings for the Query Service. 

[NOTE]
====
* This catalog is available only in Couchbase Server 8.5 and later. 
* Currently, `plan_stability` is the only setting available in the catalog. 
It controls the xref:n1ql:n1ql-manage/query-plan-stability.adoc[] feature.
====

=== View Settings

To view the current settings, run the following query:
...
The `system:settings` catalog contains the following fields:
...
| **plan_stability** +
__required__
| Settings for the xref:n1ql:n1ql-manage/query-plan-stability.adoc[] feature.
| <<sys-settings-plan-stability,Plan Stability>>
|===

[[sys-settings-plan-stability]]
==== Plan Stability
...
|**mode** +
__required__
|The plan stability mode.

Can have one of the following values: 

* `"off"`: Keeps prepared statement plans in-memory without saving them to persistent storage.
* `"prepared_only"`: Saves execution plans to persistent storage only for prepared statements.
* `"ad_hoc"`: Saves execution plans to persistent storage for all queries, including both ad-hoc and prepared statements. 
* `"ad_hoc_read_only"`: Does not save execution plans to persistent storage for new queries, but uses existing saved plans for matching queries.

|String

|**error_policy** +
__required__
|The error policy applied when saved plans encounter errors. 

Can have one of the following values:

* `"strict"`: Stops execution and returns the error to the application. 
* `"moderate"`: Generates a new plan to execute the statement, but keeps the original saved plan intact.
...
To change a setting, run an UPDATE statement against `system:settings`.

For example, to turn off Plan Stability:
...
To set the Plan Stability error policy to moderate:
...
For more information about Plan Stability modes and error policies, see xref:n1ql:n1ql-manage/query-plan-stability.adoc[].
