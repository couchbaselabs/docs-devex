...
A Capella serverless database has one auto-generated bucket, with a name derived from the database name.
The bucket contains at least one scope by default, and you can create more scopes as required.
...
For example, `default:{backtick}travel-db-mo48h2{backtick}.samples.airline` refers to the `airline` collection in the `samples` scope in the `travel-db-abc123` bucket in the `travel-db` database.
...
So for example, we may refer to the `travel-db-abc123` keyspace, meaning the default collection in the default scope in the `` bucket.
+
