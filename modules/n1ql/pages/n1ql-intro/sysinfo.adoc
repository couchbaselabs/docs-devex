...
The `system:settings` catalog contains cluster-wide settings for the xref:n1ql:n1ql-manage/query-plan-stability.adoc[Query Plan Stability] feature. 

=== View Settings

To view the current Plan Stability settings, run the following query:
...
* `"off"`: Keeps prepared statement plans in-memory without saving them to disk.
* `"prepared_only"`: Saves execution plans to disk only for prepared statements.
* `"ad_hoc"`: Saves execution plans to disk for all queries, including both ad-hoc and prepared statements. 
* `"ad_hoc_read_only`: Does not save execution plans to disk for new queries, but uses existing saved plans for matching queries.
...
For more information about Plan Stability modes and error policies, see xref:n1ql:n1ql-manage/query-plan-stability.adoc[Query Plan Stability].
