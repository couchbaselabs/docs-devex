...
****
The examples on this page use the `travel-sample` dataset, which which is supplied with Couchbase Server.
...
If your keyspace has a password, you can pass the keyspace name and keyspace password as part of the cluster or node URL.
...
+
--
To access a secure keyspace on startup, use the `-e` or `--engine` command line option, and specify the username and password in the URL.
...
+
--
To access a secure keyspace during a session, use the `\CONNECT` shell command, and specify the username and password in the URL.
...
For information on passing the user name and password credentials using other mechanisms, see <<cbq-multiple-creds>>.
...
Using the cbq shell, you can set the credentials for different users on startup or use the `\SET` shell command to set the credentials query parameter.
...
cbq runs the commands present in the input file and prints the result to stdout.
