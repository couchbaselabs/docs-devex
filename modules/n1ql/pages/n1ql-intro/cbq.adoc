...
:tabs:
:page-partial:
:page-aliases: tools:cbq-shell.adoc

[abstract]
{description}
It is a powerful, developer friendly tool that enables you to query and update data from Couchbase Server.
The cbq shell enables you to perform all the operations that are supported by the Query Workbench and more, such as additional scripting functionality.
...
The examples on this page use the `travel-sample` dataset, which is supplied with Couchbase Server.
For instructions on how to install the sample data, see xref:server:manage:manage-settings/install-sample-buckets.adoc[Sample Buckets].
...
For more information about connecting to a node or cluster, see <<cbq-connect-to-cluster>>, <<cbq-single-cred>>, and <<cbq-encrypted>>.
...
The cbq shell supports the `http://`, `https://`, `couchbase://` and `couchbases://` protocol schemes.
The `https://` and `couchbases://` protocol schemes are encrypted.
...
The `couchbase://` and `couchbases://` protocol schemes support the domain name service (DNS).
...
You may optionally specify the port when using the `http://` or `https://` protocol schemes.
...
You can close the connection with an existing node or cluster during the session without exiting the shell using the [.cmd]`\DISCONNECT` command.
...
However, this method may be useful for specifying the username and password when connecting to the cluster or node within a cbq session.
...
The following example starts cbq, assuming that you have set the password for the `travel-sample` keyspace to `w1fg2Uhj89`.
(By default, this keyspace has no password.)
...
The following example starts cbq, using the Administrator username and password.
...
The following example connects to a cluster, assuming that you have set the password for the `travel-sample` keyspace to `w1fg2Uhj89`.
(By default, this keyspace has no password.)

.Example
[source,sqlpp]
----
\CONNECT http://travel-sample:w1fg2Uhj89@localhost:8091;
----

The following example connects to a cluster, using the Administrator username and password.

.Example
[source,sqlpp]
----
\CONNECT http://Administrator:password@localhost:8091;
----
--
====
...
+
--
To set the credentials for different users on startup, use the `-c` or `--credentials` command line option, followed by the list of credentials.
...
[source,sh]
----
./cbq -e http://localhost:8091 -c=travel-sample:pwd1,beer-sample:pwd2
----
--
...
+
--
To switch between users and change credentials during a session, use the `\SET` shell command to specify the [.param]`-creds` query parameter.
...
You can connect to the cluster or node with an encrypted protocol scheme -- that is, either `https://` or `couchbases://`.
...
You can use the encrypted `couchbases://` protocol scheme with a domain name to connect to a node or cluster deployed in Couchbase Capella.
...
If specified, when you connect to a cluster, cbq automatically discovers and connects to an Analytics node.
...
`--cacert`
| string (path)
a| Only applicable when using an encrypted protocol scheme -- either `https://` or `couchbases://`.
...
[source,sqlpp]
----
\CONNECT http://localhost:8093;
----

[source,sqlpp]
----
\CONNECT http://[fd63:6f75:6368:1075:816:3c1d:789b:bc4]:8091;
----
