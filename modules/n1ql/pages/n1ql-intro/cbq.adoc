...
It is also available in the xref:cli:cli-intro.adoc#server-tools-packages[Couchbase Server tools] package beginning with version 7.6.2.
...
For example: `localhost`, `node1`, `127.0.0.1`, or `[fd63:6f75:6368:1075:816:3c1d:789b:bc4]`.
...
You can close the connection with a node or cluster during the session without exiting the shell using the [.cmd]`\DISCONNECT` command.
...
You can bring up the shell without connecting to any query service or cluster node by using the [.opt]`-ne` or [.opt]`--no-engine` option.
After starting cbq without any connection, you can connect to a node using the [.cmd]`\CONNECT` command.
...
For more information about using multiple credentials, see xref:n1ql:n1ql-rest-api/index.adoc#creds[Query Service REST API] and xref:n1ql:n1ql-rest-api/exauthrequest.adoc[].
...
You cannot specify the port when using the `couchbases://` protocol scheme.
...
[source,sh]
----
./cbq -c=$USER:$PASSWORD,$USER2:$PASSWORD2 \
-e $BASE_URL
----
...
To display the version of the query engine (this is not the same as the version of Couchbase Server itself), use one of the following {sqlpp} queries:
