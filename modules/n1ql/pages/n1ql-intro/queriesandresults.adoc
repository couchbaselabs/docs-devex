...
// Needs updating to point to Elixir version of that page.
See xref:clusters:query-service/query-workbench.adoc#specify-bucket-and-scope-context[Query Workbench] for how to set the Query Context in a Capella Serverless database.
...
You can set query parameter values when you run the query, using
ifdef::flag-devex-command-line[]
the cbq query shell,
endif::flag-devex-command-line[]
the Query Workbench, or the {sqlpp} REST API.
