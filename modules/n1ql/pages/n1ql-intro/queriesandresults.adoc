...
When a {sqlpp} query string is sent to the server, the server inspects the string and parses it, planning which indexes to query.
...
The Couchbase Query service uses _indexes_ to do its work.
