...
The [dfn]#query context# specifies the namespace, bucket, and scope used to resolve partial keyspace references.
...
* To set the query context in the Query Workbench, use the xref:tools:query-workbench.adoc#query-context[query context] drop-down menu in the Query Editor.
...
For more information, refer to xref:indexes:indexing-overview.adoc[].
