...
This tutorial teaches you about the key concepts behind Couchbase and how they differ from traditional SQL database systems like https://www.mysql.com/[MySQL^] and https://www.oracle.com/ca-en/database/technologies/[Oracle NoSQL^].

IMPORTANT: This tutorial is designed for use with a Capella operational cluster.
To use a standalone or Docker installation of Couchbase, see the xref:server:tutorials:couchbase-tutorial-student-records.adoc[Couchbase Server Developer Tutorial]. 
...
*course*:: Courses the students can take.
Includes course name, faculty, and the number of credit points associated with the course. 
Students can take more than 1 course at a time.
...
In a relational model database, the data for this example contains a list of students and a list of courses. 
...
The `enrollment` table highlights a challenge with the relational model.
...
If you create the same data using the document model supported by Couchbase databases, each record is stored as a JSON document. 
The document model:
...
This structure lets you access and retrieve all of Hilary's details with 1 search and without the need for complex table joins.
...
[horizontal]
Step 1:: xref:get-started:create-account.adoc[]
...
For troubleshooting information, see xref:java-tutorial/tutorial-troubleshooting.adoc[].