...
Before establishing a connection between the Java SDK and your cluster, you must perform the following steps to obtain the connection string and configure cluster credentials and IP address: 

Before connecting the Java SDK to your cluster, you must obtain the connection string and set from the Capella Operational UI. 

To get the connection string: 

. On the *Operational Clusters* page, select *student-cluster*.

. Go to menu:Connect[SDK].

. Note the public connection string. This is required to connect to the cluster.

. follow the instructions on screen to enter an allowed IP address.

. Choose an existing cluster user from the drop-down, or create a new cluster access user.


Once these steps are complete, you can proceed with connecting the Java SDK to your cluster. 
