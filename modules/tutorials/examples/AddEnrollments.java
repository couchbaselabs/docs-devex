...
public class AddEnrollments {
    ...
    public static void main(String[] args) {
        ...
        // Retrieves Hilary's student record, the `graphic design` course record, and the `art history` course record. Each method uses a SQL++ call to retrieve a single record from each collection.
        ...
        // Stores the `enrollments` inside the student record as an array. `JsonArray.create()` creates an empty list structure.
        JsonArray enrollments = JsonArray.create();    

        // Adds two JSON elements to the `enrollments` array: the course that the enrollment relates to, and the date that the student enrolled in the course. To avoid repeating data all over the cluster, you store a reference to the course instead of the entire course record itself in this field. This means that you do not have to search through every single record if the course details change
        ...
        // Commits the changes to the collection. The `upsert` function call takes the key of the record you want to insert or update and the record itself as parameters. If the `upsert` call finds a document with a matching ID in the collection, it updates the document. If there is no matching ID, it creates a new document.
